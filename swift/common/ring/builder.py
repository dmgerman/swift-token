begin_unit
comment|'# Copyright (c) 2010-2012 OpenStack Foundation'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# Licensed under the Apache License, Version 2.0 (the "License");'
nl|'\n'
comment|'# you may not use this file except in compliance with the License.'
nl|'\n'
comment|'# You may obtain a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'# distributed under the License is distributed on an "AS IS" BASIS,'
nl|'\n'
comment|'# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or'
nl|'\n'
comment|'# implied.'
nl|'\n'
comment|'# See the License for the specific language governing permissions and'
nl|'\n'
comment|'# limitations under the License.'
nl|'\n'
nl|'\n'
name|'import'
name|'copy'
newline|'\n'
name|'import'
name|'errno'
newline|'\n'
name|'import'
name|'itertools'
newline|'\n'
name|'import'
name|'logging'
newline|'\n'
name|'import'
name|'math'
newline|'\n'
name|'import'
name|'random'
newline|'\n'
name|'import'
name|'six'
op|'.'
name|'moves'
op|'.'
name|'cPickle'
name|'as'
name|'pickle'
newline|'\n'
name|'from'
name|'copy'
name|'import'
name|'deepcopy'
newline|'\n'
name|'from'
name|'contextlib'
name|'import'
name|'contextmanager'
newline|'\n'
nl|'\n'
name|'from'
name|'array'
name|'import'
name|'array'
newline|'\n'
name|'from'
name|'collections'
name|'import'
name|'defaultdict'
newline|'\n'
name|'import'
name|'six'
newline|'\n'
name|'from'
name|'six'
op|'.'
name|'moves'
name|'import'
name|'range'
newline|'\n'
name|'from'
name|'time'
name|'import'
name|'time'
newline|'\n'
nl|'\n'
name|'from'
name|'swift'
op|'.'
name|'common'
name|'import'
name|'exceptions'
newline|'\n'
name|'from'
name|'swift'
op|'.'
name|'common'
op|'.'
name|'ring'
name|'import'
name|'RingData'
newline|'\n'
name|'from'
name|'swift'
op|'.'
name|'common'
op|'.'
name|'ring'
op|'.'
name|'utils'
name|'import'
name|'tiers_for_dev'
op|','
name|'build_tier_tree'
op|','
name|'validate_and_normalize_address'
newline|'\n'
nl|'\n'
comment|"# we can't store None's in the replica2part2dev array, so we high-jack"
nl|'\n'
comment|'# the max value for magic to represent the part is not currently'
nl|'\n'
comment|'# assigned to any device.'
nl|'\n'
DECL|variable|NONE_DEV
name|'NONE_DEV'
op|'='
number|'2'
op|'**'
number|'16'
op|'-'
number|'1'
newline|'\n'
DECL|variable|MAX_BALANCE
name|'MAX_BALANCE'
op|'='
number|'999.99'
newline|'\n'
DECL|variable|MAX_BALANCE_GATHER_COUNT
name|'MAX_BALANCE_GATHER_COUNT'
op|'='
number|'3'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|RingValidationWarning
name|'class'
name|'RingValidationWarning'
op|'('
name|'Warning'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'pass'
newline|'\n'
nl|'\n'
nl|'\n'
dedent|''
name|'try'
op|':'
newline|'\n'
comment|'# python 2.7+'
nl|'\n'
indent|'    '
name|'from'
name|'logging'
name|'import'
name|'NullHandler'
newline|'\n'
dedent|''
name|'except'
name|'ImportError'
op|':'
newline|'\n'
comment|'# python 2.6'
nl|'\n'
DECL|class|NullHandler
indent|'    '
name|'class'
name|'NullHandler'
op|'('
name|'logging'
op|'.'
name|'Handler'
op|')'
op|':'
newline|'\n'
DECL|member|emit
indent|'        '
name|'def'
name|'emit'
op|'('
name|'self'
op|','
op|'*'
name|'a'
op|','
op|'**'
name|'kw'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'pass'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|RingBuilder
dedent|''
dedent|''
dedent|''
name|'class'
name|'RingBuilder'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""\n    Used to build swift.common.ring.RingData instances to be written to disk\n    and used with swift.common.ring.Ring instances. See bin/swift-ring-builder\n    for example usage.\n\n    The instance variable devs_changed indicates if the device information has\n    changed since the last balancing. This can be used by tools to know whether\n    a rebalance request is an isolated request or due to added, changed, or\n    removed devices.\n\n    :param part_power: number of partitions = 2**part_power.\n    :param replicas: number of replicas for each partition\n    :param min_part_hours: minimum number of hours between partition changes\n    """'
newline|'\n'
nl|'\n'
DECL|member|__init__
name|'def'
name|'__init__'
op|'('
name|'self'
op|','
name|'part_power'
op|','
name|'replicas'
op|','
name|'min_part_hours'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'part_power'
op|'>'
number|'32'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'ValueError'
op|'('
string|'"part_power must be at most 32 (was %d)"'
nl|'\n'
op|'%'
op|'('
name|'part_power'
op|','
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'replicas'
op|'<'
number|'1'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'ValueError'
op|'('
string|'"replicas must be at least 1 (was %.6f)"'
nl|'\n'
op|'%'
op|'('
name|'replicas'
op|','
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'min_part_hours'
op|'<'
number|'0'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'ValueError'
op|'('
string|'"min_part_hours must be non-negative (was %d)"'
nl|'\n'
op|'%'
op|'('
name|'min_part_hours'
op|','
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'self'
op|'.'
name|'part_power'
op|'='
name|'part_power'
newline|'\n'
name|'self'
op|'.'
name|'replicas'
op|'='
name|'replicas'
newline|'\n'
name|'self'
op|'.'
name|'min_part_hours'
op|'='
name|'min_part_hours'
newline|'\n'
name|'self'
op|'.'
name|'parts'
op|'='
number|'2'
op|'**'
name|'self'
op|'.'
name|'part_power'
newline|'\n'
name|'self'
op|'.'
name|'devs'
op|'='
op|'['
op|']'
newline|'\n'
name|'self'
op|'.'
name|'devs_changed'
op|'='
name|'False'
newline|'\n'
name|'self'
op|'.'
name|'version'
op|'='
number|'0'
newline|'\n'
name|'self'
op|'.'
name|'overload'
op|'='
number|'0.0'
newline|'\n'
nl|'\n'
comment|'# _replica2part2dev maps from replica number to partition number to'
nl|'\n'
comment|"# device id. So, for a three replica, 2**23 ring, it's an array of"
nl|'\n'
comment|'# three 2**23 arrays of device ids (unsigned shorts). This can work a'
nl|'\n'
comment|'# bit faster than the 2**23 array of triplet arrays of device ids in'
nl|'\n'
comment|"# many circumstances. Making one big 2**23 * 3 array didn't seem to"
nl|'\n'
comment|"# have any speed change; though you're welcome to try it again (it was"
nl|'\n'
comment|'# a while ago, code-wise, when I last tried it).'
nl|'\n'
name|'self'
op|'.'
name|'_replica2part2dev'
op|'='
name|'None'
newline|'\n'
nl|'\n'
comment|'# _last_part_moves is an array of unsigned bytes representing'
nl|'\n'
comment|'# the number of hours since a given partition was last moved.'
nl|'\n'
comment|"# This is used to guarantee we don't move a partition twice"
nl|'\n'
comment|'# within a given number of hours (24 is my usual test). Removing'
nl|'\n'
comment|"# a device overrides this behavior as it's assumed that's only"
nl|'\n'
comment|'# done because of device failure.'
nl|'\n'
name|'self'
op|'.'
name|'_last_part_moves'
op|'='
name|'None'
newline|'\n'
comment|'# _last_part_moves_epoch indicates the time the offsets in'
nl|'\n'
comment|'# _last_part_moves is based on.'
nl|'\n'
name|'self'
op|'.'
name|'_last_part_moves_epoch'
op|'='
number|'0'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_last_part_gather_start'
op|'='
number|'0'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_dispersion_graph'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'self'
op|'.'
name|'dispersion'
op|'='
number|'0.0'
newline|'\n'
name|'self'
op|'.'
name|'_remove_devs'
op|'='
op|'['
op|']'
newline|'\n'
name|'self'
op|'.'
name|'_ring'
op|'='
name|'None'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'logger'
op|'='
name|'logging'
op|'.'
name|'getLogger'
op|'('
string|'"swift.ring.builder"'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'handlers'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'logger'
op|'.'
name|'disabled'
op|'='
name|'True'
newline|'\n'
comment|'# silence "no handler for X" error messages'
nl|'\n'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'addHandler'
op|'('
name|'NullHandler'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'contextmanager'
newline|'\n'
DECL|member|debug
name|'def'
name|'debug'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Temporarily enables debug logging, useful in tests, e.g.\n\n            with rb.debug():\n                rb.rebalance()\n        """'
newline|'\n'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'disabled'
op|'='
name|'False'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'yield'
newline|'\n'
dedent|''
name|'finally'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'logger'
op|'.'
name|'disabled'
op|'='
name|'True'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'property'
newline|'\n'
DECL|member|min_part_seconds_left
name|'def'
name|'min_part_seconds_left'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Get the total seconds until a rebalance can be performed"""'
newline|'\n'
name|'elapsed_seconds'
op|'='
name|'int'
op|'('
name|'time'
op|'('
op|')'
op|'-'
name|'self'
op|'.'
name|'_last_part_moves_epoch'
op|')'
newline|'\n'
name|'return'
name|'max'
op|'('
op|'('
name|'self'
op|'.'
name|'min_part_hours'
op|'*'
number|'3600'
op|')'
op|'-'
name|'elapsed_seconds'
op|','
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|member|weight_of_one_part
dedent|''
name|'def'
name|'weight_of_one_part'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Returns the weight of each partition as calculated from the\n        total weight of all the devices.\n        """'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'self'
op|'.'
name|'parts'
op|'*'
name|'self'
op|'.'
name|'replicas'
op|'/'
name|'sum'
op|'('
name|'d'
op|'['
string|"'weight'"
op|']'
name|'for'
name|'d'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'ZeroDivisionError'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exceptions'
op|'.'
name|'EmptyRingError'
op|'('
string|"'There are no devices in this '"
nl|'\n'
string|"'ring, or all devices have been '"
nl|'\n'
string|"'deleted'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'classmethod'
newline|'\n'
DECL|member|from_dict
name|'def'
name|'from_dict'
op|'('
name|'cls'
op|','
name|'builder_data'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'b'
op|'='
name|'cls'
op|'('
number|'1'
op|','
number|'1'
op|','
number|'1'
op|')'
comment|'# Dummy values'
newline|'\n'
name|'b'
op|'.'
name|'copy_from'
op|'('
name|'builder_data'
op|')'
newline|'\n'
name|'return'
name|'b'
newline|'\n'
nl|'\n'
DECL|member|copy_from
dedent|''
name|'def'
name|'copy_from'
op|'('
name|'self'
op|','
name|'builder'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Reinitializes this RingBuilder instance from data obtained from the\n        builder dict given. Code example::\n\n            b = RingBuilder(1, 1, 1)  # Dummy values\n            b.copy_from(builder)\n\n        This is to restore a RingBuilder that has had its b.to_dict()\n        previously saved.\n        """'
newline|'\n'
name|'if'
name|'hasattr'
op|'('
name|'builder'
op|','
string|"'devs'"
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'part_power'
op|'='
name|'builder'
op|'.'
name|'part_power'
newline|'\n'
name|'self'
op|'.'
name|'replicas'
op|'='
name|'builder'
op|'.'
name|'replicas'
newline|'\n'
name|'self'
op|'.'
name|'min_part_hours'
op|'='
name|'builder'
op|'.'
name|'min_part_hours'
newline|'\n'
name|'self'
op|'.'
name|'parts'
op|'='
name|'builder'
op|'.'
name|'parts'
newline|'\n'
name|'self'
op|'.'
name|'devs'
op|'='
name|'builder'
op|'.'
name|'devs'
newline|'\n'
name|'self'
op|'.'
name|'devs_changed'
op|'='
name|'builder'
op|'.'
name|'devs_changed'
newline|'\n'
name|'self'
op|'.'
name|'overload'
op|'='
name|'builder'
op|'.'
name|'overload'
newline|'\n'
name|'self'
op|'.'
name|'version'
op|'='
name|'builder'
op|'.'
name|'version'
newline|'\n'
name|'self'
op|'.'
name|'_replica2part2dev'
op|'='
name|'builder'
op|'.'
name|'_replica2part2dev'
newline|'\n'
name|'self'
op|'.'
name|'_last_part_moves_epoch'
op|'='
name|'builder'
op|'.'
name|'_last_part_moves_epoch'
newline|'\n'
name|'self'
op|'.'
name|'_last_part_moves'
op|'='
name|'builder'
op|'.'
name|'_last_part_moves'
newline|'\n'
name|'self'
op|'.'
name|'_last_part_gather_start'
op|'='
name|'builder'
op|'.'
name|'_last_part_gather_start'
newline|'\n'
name|'self'
op|'.'
name|'_remove_devs'
op|'='
name|'builder'
op|'.'
name|'_remove_devs'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'part_power'
op|'='
name|'builder'
op|'['
string|"'part_power'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'replicas'
op|'='
name|'builder'
op|'['
string|"'replicas'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'min_part_hours'
op|'='
name|'builder'
op|'['
string|"'min_part_hours'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'parts'
op|'='
name|'builder'
op|'['
string|"'parts'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'devs'
op|'='
name|'builder'
op|'['
string|"'devs'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'devs_changed'
op|'='
name|'builder'
op|'['
string|"'devs_changed'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'overload'
op|'='
name|'builder'
op|'.'
name|'get'
op|'('
string|"'overload'"
op|','
number|'0.0'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'version'
op|'='
name|'builder'
op|'['
string|"'version'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'_replica2part2dev'
op|'='
name|'builder'
op|'['
string|"'_replica2part2dev'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'_last_part_moves_epoch'
op|'='
name|'builder'
op|'['
string|"'_last_part_moves_epoch'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'_last_part_moves'
op|'='
name|'builder'
op|'['
string|"'_last_part_moves'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'_last_part_gather_start'
op|'='
name|'builder'
op|'['
string|"'_last_part_gather_start'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'_dispersion_graph'
op|'='
name|'builder'
op|'.'
name|'get'
op|'('
string|"'_dispersion_graph'"
op|','
op|'{'
op|'}'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'dispersion'
op|'='
name|'builder'
op|'.'
name|'get'
op|'('
string|"'dispersion'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_remove_devs'
op|'='
name|'builder'
op|'['
string|"'_remove_devs'"
op|']'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'_ring'
op|'='
name|'None'
newline|'\n'
nl|'\n'
comment|'# Old builders may not have a region defined for their devices, in'
nl|'\n'
comment|'# which case we default it to 1.'
nl|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'dev'
op|'.'
name|'setdefault'
op|'('
string|'"region"'
op|','
number|'1'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'not'
name|'self'
op|'.'
name|'_last_part_moves_epoch'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'_last_part_moves_epoch'
op|'='
number|'0'
newline|'\n'
nl|'\n'
DECL|member|__deepcopy__
dedent|''
dedent|''
name|'def'
name|'__deepcopy__'
op|'('
name|'self'
op|','
name|'memo'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'type'
op|'('
name|'self'
op|')'
op|'.'
name|'from_dict'
op|'('
name|'deepcopy'
op|'('
name|'self'
op|'.'
name|'to_dict'
op|'('
op|')'
op|','
name|'memo'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|to_dict
dedent|''
name|'def'
name|'to_dict'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Returns a dict that can be used later with copy_from to\n        restore a RingBuilder. swift-ring-builder uses this to\n        pickle.dump the dict to a file and later load that dict into\n        copy_from.\n        """'
newline|'\n'
name|'return'
op|'{'
string|"'part_power'"
op|':'
name|'self'
op|'.'
name|'part_power'
op|','
nl|'\n'
string|"'replicas'"
op|':'
name|'self'
op|'.'
name|'replicas'
op|','
nl|'\n'
string|"'min_part_hours'"
op|':'
name|'self'
op|'.'
name|'min_part_hours'
op|','
nl|'\n'
string|"'parts'"
op|':'
name|'self'
op|'.'
name|'parts'
op|','
nl|'\n'
string|"'devs'"
op|':'
name|'self'
op|'.'
name|'devs'
op|','
nl|'\n'
string|"'devs_changed'"
op|':'
name|'self'
op|'.'
name|'devs_changed'
op|','
nl|'\n'
string|"'version'"
op|':'
name|'self'
op|'.'
name|'version'
op|','
nl|'\n'
string|"'overload'"
op|':'
name|'self'
op|'.'
name|'overload'
op|','
nl|'\n'
string|"'_replica2part2dev'"
op|':'
name|'self'
op|'.'
name|'_replica2part2dev'
op|','
nl|'\n'
string|"'_last_part_moves_epoch'"
op|':'
name|'self'
op|'.'
name|'_last_part_moves_epoch'
op|','
nl|'\n'
string|"'_last_part_moves'"
op|':'
name|'self'
op|'.'
name|'_last_part_moves'
op|','
nl|'\n'
string|"'_last_part_gather_start'"
op|':'
name|'self'
op|'.'
name|'_last_part_gather_start'
op|','
nl|'\n'
string|"'_dispersion_graph'"
op|':'
name|'self'
op|'.'
name|'_dispersion_graph'
op|','
nl|'\n'
string|"'dispersion'"
op|':'
name|'self'
op|'.'
name|'dispersion'
op|','
nl|'\n'
string|"'_remove_devs'"
op|':'
name|'self'
op|'.'
name|'_remove_devs'
op|'}'
newline|'\n'
nl|'\n'
DECL|member|change_min_part_hours
dedent|''
name|'def'
name|'change_min_part_hours'
op|'('
name|'self'
op|','
name|'min_part_hours'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Changes the value used to decide if a given partition can be moved\n        again. This restriction is to give the overall system enough time to\n        settle a partition to its new location before moving it to yet another\n        location. While no data would be lost if a partition is moved several\n        times quickly, it could make that data unreachable for a short period\n        of time.\n\n        This should be set to at least the average full partition replication\n        time. Starting it at 24 hours and then lowering it to what the\n        replicator reports as the longest partition cycle is best.\n\n        :param min_part_hours: new value for min_part_hours\n        """'
newline|'\n'
name|'self'
op|'.'
name|'min_part_hours'
op|'='
name|'min_part_hours'
newline|'\n'
nl|'\n'
DECL|member|set_replicas
dedent|''
name|'def'
name|'set_replicas'
op|'('
name|'self'
op|','
name|'new_replica_count'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Changes the number of replicas in this ring.\n\n        If the new replica count is sufficiently different that\n        self._replica2part2dev will change size, sets\n        self.devs_changed. This is so tools like\n        bin/swift-ring-builder can know to write out the new ring\n        rather than bailing out due to lack of balance change.\n        """'
newline|'\n'
name|'old_slots_used'
op|'='
name|'int'
op|'('
name|'self'
op|'.'
name|'parts'
op|'*'
name|'self'
op|'.'
name|'replicas'
op|')'
newline|'\n'
name|'new_slots_used'
op|'='
name|'int'
op|'('
name|'self'
op|'.'
name|'parts'
op|'*'
name|'new_replica_count'
op|')'
newline|'\n'
name|'if'
name|'old_slots_used'
op|'!='
name|'new_slots_used'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'devs_changed'
op|'='
name|'True'
newline|'\n'
nl|'\n'
dedent|''
name|'self'
op|'.'
name|'replicas'
op|'='
name|'new_replica_count'
newline|'\n'
nl|'\n'
DECL|member|set_overload
dedent|''
name|'def'
name|'set_overload'
op|'('
name|'self'
op|','
name|'overload'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'overload'
op|'='
name|'overload'
newline|'\n'
nl|'\n'
DECL|member|get_ring
dedent|''
name|'def'
name|'get_ring'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Get the ring, or more specifically, the swift.common.ring.RingData.\n        This ring data is the minimum required for use of the ring. The ring\n        builder itself keeps additional data such as when partitions were last\n        moved.\n        """'
newline|'\n'
comment|"# We cache the self._ring value so multiple requests for it don't build"
nl|'\n'
comment|'# it multiple times. Be sure to set self._ring = None whenever the ring'
nl|'\n'
comment|'# will need to be rebuilt.'
nl|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'_ring'
op|':'
newline|'\n'
comment|'# Make devs list (with holes for deleted devices) and not including'
nl|'\n'
comment|'# builder-specific extra attributes.'
nl|'\n'
indent|'            '
name|'devs'
op|'='
op|'['
name|'None'
op|']'
op|'*'
name|'len'
op|'('
name|'self'
op|'.'
name|'devs'
op|')'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                '
name|'devs'
op|'['
name|'dev'
op|'['
string|"'id'"
op|']'
op|']'
op|'='
name|'dict'
op|'('
op|'('
name|'k'
op|','
name|'v'
op|')'
name|'for'
name|'k'
op|','
name|'v'
name|'in'
name|'dev'
op|'.'
name|'items'
op|'('
op|')'
nl|'\n'
name|'if'
name|'k'
name|'not'
name|'in'
op|'('
string|"'parts'"
op|','
string|"'parts_wanted'"
op|')'
op|')'
newline|'\n'
comment|'# Copy over the replica+partition->device assignments, the device'
nl|'\n'
comment|'# information, and the part_shift value (the number of bits to'
nl|'\n'
comment|'# shift an unsigned int >I right to obtain the partition for the'
nl|'\n'
comment|'# int).'
nl|'\n'
dedent|''
name|'if'
name|'not'
name|'self'
op|'.'
name|'_replica2part2dev'
op|':'
newline|'\n'
indent|'                '
name|'self'
op|'.'
name|'_ring'
op|'='
name|'RingData'
op|'('
op|'['
op|']'
op|','
name|'devs'
op|','
number|'32'
op|'-'
name|'self'
op|'.'
name|'part_power'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'self'
op|'.'
name|'_ring'
op|'='
name|'RingData'
op|'('
op|'['
name|'array'
op|'('
string|"'H'"
op|','
name|'p2d'
op|')'
name|'for'
name|'p2d'
name|'in'
nl|'\n'
name|'self'
op|'.'
name|'_replica2part2dev'
op|']'
op|','
nl|'\n'
name|'devs'
op|','
number|'32'
op|'-'
name|'self'
op|'.'
name|'part_power'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'self'
op|'.'
name|'_ring'
newline|'\n'
nl|'\n'
DECL|member|add_dev
dedent|''
name|'def'
name|'add_dev'
op|'('
name|'self'
op|','
name|'dev'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Add a device to the ring. This device dict should have a minimum of the\n        following keys:\n\n        ======  ===============================================================\n        id      unique integer identifier amongst devices. Defaults to the next\n                id if the \'id\' key is not provided in the dict\n        weight  a float of the relative weight of this device as compared to\n                others; this indicates how many partitions the builder will try\n                to assign to this device\n        region  integer indicating which region the device is in\n        zone    integer indicating which zone the device is in; a given\n                partition will not be assigned to multiple devices within the\n                same (region, zone) pair if there is any alternative\n        ip      the ip address of the device\n        port    the tcp port of the device\n        device  the device\'s name on disk (sdb1, for example)\n        meta    general use \'extra\' field; for example: the online date, the\n                hardware description\n        ======  ===============================================================\n\n        .. note::\n            This will not rebalance the ring immediately as you may want to\n            make multiple changes for a single rebalance.\n\n        :param dev: device dict\n\n        :returns: id of device (not used in the tree anymore, but unknown\n                  users may depend on it)\n        """'
newline|'\n'
name|'if'
string|"'id'"
name|'not'
name|'in'
name|'dev'
op|':'
newline|'\n'
indent|'            '
name|'dev'
op|'['
string|"'id'"
op|']'
op|'='
number|'0'
newline|'\n'
name|'if'
name|'self'
op|'.'
name|'devs'
op|':'
newline|'\n'
indent|'                '
name|'try'
op|':'
newline|'\n'
indent|'                    '
name|'dev'
op|'['
string|"'id'"
op|']'
op|'='
name|'self'
op|'.'
name|'devs'
op|'.'
name|'index'
op|'('
name|'None'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'ValueError'
op|':'
newline|'\n'
indent|'                    '
name|'dev'
op|'['
string|"'id'"
op|']'
op|'='
name|'len'
op|'('
name|'self'
op|'.'
name|'devs'
op|')'
newline|'\n'
dedent|''
dedent|''
dedent|''
name|'if'
name|'dev'
op|'['
string|"'id'"
op|']'
op|'<'
name|'len'
op|'('
name|'self'
op|'.'
name|'devs'
op|')'
name|'and'
name|'self'
op|'.'
name|'devs'
op|'['
name|'dev'
op|'['
string|"'id'"
op|']'
op|']'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exceptions'
op|'.'
name|'DuplicateDeviceError'
op|'('
nl|'\n'
string|"'Duplicate device id: %d'"
op|'%'
name|'dev'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
comment|"# Add holes to self.devs to ensure self.devs[dev['id']] will be the dev"
nl|'\n'
dedent|''
name|'while'
name|'dev'
op|'['
string|"'id'"
op|']'
op|'>='
name|'len'
op|'('
name|'self'
op|'.'
name|'devs'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'devs'
op|'.'
name|'append'
op|'('
name|'None'
op|')'
newline|'\n'
dedent|''
name|'dev'
op|'['
string|"'weight'"
op|']'
op|'='
name|'float'
op|'('
name|'dev'
op|'['
string|"'weight'"
op|']'
op|')'
newline|'\n'
name|'dev'
op|'['
string|"'parts'"
op|']'
op|'='
number|'0'
newline|'\n'
name|'self'
op|'.'
name|'devs'
op|'['
name|'dev'
op|'['
string|"'id'"
op|']'
op|']'
op|'='
name|'dev'
newline|'\n'
name|'self'
op|'.'
name|'devs_changed'
op|'='
name|'True'
newline|'\n'
name|'self'
op|'.'
name|'version'
op|'+='
number|'1'
newline|'\n'
name|'return'
name|'dev'
op|'['
string|"'id'"
op|']'
newline|'\n'
nl|'\n'
DECL|member|set_dev_weight
dedent|''
name|'def'
name|'set_dev_weight'
op|'('
name|'self'
op|','
name|'dev_id'
op|','
name|'weight'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Set the weight of a device. This should be called rather than just\n        altering the weight key in the device dict directly, as the builder\n        will need to rebuild some internal state to reflect the change.\n\n        .. note::\n            This will not rebalance the ring immediately as you may want to\n            make multiple changes for a single rebalance.\n\n        :param dev_id: device id\n        :param weight: new weight for device\n        """'
newline|'\n'
name|'if'
name|'any'
op|'('
name|'dev_id'
op|'=='
name|'d'
op|'['
string|"'id'"
op|']'
name|'for'
name|'d'
name|'in'
name|'self'
op|'.'
name|'_remove_devs'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'ValueError'
op|'('
string|'"Can not set weight of dev_id %s because it "'
nl|'\n'
string|'"is marked for removal"'
op|'%'
op|'('
name|'dev_id'
op|','
op|')'
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'devs'
op|'['
name|'dev_id'
op|']'
op|'['
string|"'weight'"
op|']'
op|'='
name|'weight'
newline|'\n'
name|'self'
op|'.'
name|'devs_changed'
op|'='
name|'True'
newline|'\n'
name|'self'
op|'.'
name|'version'
op|'+='
number|'1'
newline|'\n'
nl|'\n'
DECL|member|remove_dev
dedent|''
name|'def'
name|'remove_dev'
op|'('
name|'self'
op|','
name|'dev_id'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Remove a device from the ring.\n\n        .. note::\n            This will not rebalance the ring immediately as you may want to\n            make multiple changes for a single rebalance.\n\n        :param dev_id: device id\n        """'
newline|'\n'
name|'dev'
op|'='
name|'self'
op|'.'
name|'devs'
op|'['
name|'dev_id'
op|']'
newline|'\n'
name|'dev'
op|'['
string|"'weight'"
op|']'
op|'='
number|'0'
newline|'\n'
name|'self'
op|'.'
name|'_remove_devs'
op|'.'
name|'append'
op|'('
name|'dev'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'devs_changed'
op|'='
name|'True'
newline|'\n'
name|'self'
op|'.'
name|'version'
op|'+='
number|'1'
newline|'\n'
nl|'\n'
DECL|member|rebalance
dedent|''
name|'def'
name|'rebalance'
op|'('
name|'self'
op|','
name|'seed'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Rebalance the ring.\n\n        This is the main work function of the builder, as it will assign and\n        reassign partitions to devices in the ring based on weights, distinct\n        zones, recent reassignments, etc.\n\n        The process doesn\'t always perfectly assign partitions (that\'d take a\n        lot more analysis and therefore a lot more time -- I had code that did\n        that before). Because of this, it keeps rebalancing until the device\n        skew (number of partitions a device wants compared to what it has) gets\n        below 1% or doesn\'t change by more than 1% (only happens with ring that\n        can\'t be balanced no matter what).\n\n        :returns: (number_of_partitions_altered, resulting_balance,\n                   number_of_removed_devices)\n        """'
newline|'\n'
comment|'# count up the devs, and cache some stuff'
nl|'\n'
name|'num_devices'
op|'='
number|'0'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'dev'
op|'['
string|"'tiers'"
op|']'
op|'='
name|'tiers_for_dev'
op|'('
name|'dev'
op|')'
newline|'\n'
name|'if'
name|'dev'
op|'['
string|"'weight'"
op|']'
op|'>'
number|'0'
op|':'
newline|'\n'
indent|'                '
name|'num_devices'
op|'+='
number|'1'
newline|'\n'
dedent|''
dedent|''
name|'if'
name|'num_devices'
op|'<'
name|'self'
op|'.'
name|'replicas'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|'"Replica count of %(replicas)s requires more "'
nl|'\n'
string|'"than %(num_devices)s devices"'
op|'%'
op|'{'
nl|'\n'
string|"'replicas'"
op|':'
name|'self'
op|'.'
name|'replicas'
op|','
nl|'\n'
string|"'num_devices'"
op|':'
name|'num_devices'
op|','
nl|'\n'
op|'}'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'seed'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'random'
op|'.'
name|'seed'
op|'('
name|'seed'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'self'
op|'.'
name|'_ring'
op|'='
name|'None'
newline|'\n'
nl|'\n'
name|'old_replica2part2dev'
op|'='
name|'copy'
op|'.'
name|'deepcopy'
op|'('
name|'self'
op|'.'
name|'_replica2part2dev'
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'self'
op|'.'
name|'_last_part_moves'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
string|'"New builder; performing initial balance"'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_last_part_moves'
op|'='
name|'array'
op|'('
string|"'B'"
op|','
name|'itertools'
op|'.'
name|'repeat'
op|'('
number|'0'
op|','
name|'self'
op|'.'
name|'parts'
op|')'
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'_update_last_part_moves'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'replica_plan'
op|'='
name|'self'
op|'.'
name|'_build_replica_plan'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_set_parts_wanted'
op|'('
name|'replica_plan'
op|')'
newline|'\n'
nl|'\n'
name|'assign_parts'
op|'='
name|'defaultdict'
op|'('
name|'list'
op|')'
newline|'\n'
comment|'# gather parts from failed devices'
nl|'\n'
name|'removed_devs'
op|'='
name|'self'
op|'.'
name|'_gather_parts_from_failed_devices'
op|'('
name|'assign_parts'
op|')'
newline|'\n'
comment|'# gather parts from replica count adjustment'
nl|'\n'
name|'self'
op|'.'
name|'_adjust_replica2part2dev_size'
op|'('
name|'assign_parts'
op|')'
newline|'\n'
comment|'# gather parts for dispersion (N.B. this only picks up parts that'
nl|'\n'
comment|'# *must* disperse according to the replica plan)'
nl|'\n'
name|'self'
op|'.'
name|'_gather_parts_for_dispersion'
op|'('
name|'assign_parts'
op|','
name|'replica_plan'
op|')'
newline|'\n'
nl|'\n'
comment|"# we'll gather a few times, or until we archive the plan"
nl|'\n'
name|'for'
name|'gather_count'
name|'in'
name|'range'
op|'('
name|'MAX_BALANCE_GATHER_COUNT'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'_gather_parts_for_balance'
op|'('
name|'assign_parts'
op|','
name|'replica_plan'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'assign_parts'
op|':'
newline|'\n'
comment|'# most likely min part hours'
nl|'\n'
indent|'                '
name|'finish_status'
op|'='
string|"'Unable to finish'"
newline|'\n'
name|'break'
newline|'\n'
dedent|''
name|'assign_parts_list'
op|'='
name|'list'
op|'('
name|'assign_parts'
op|'.'
name|'items'
op|'('
op|')'
op|')'
newline|'\n'
comment|'# shuffle the parts to be reassigned, we have no preference on the'
nl|'\n'
comment|'# order in which the replica plan is fulfilled.'
nl|'\n'
name|'random'
op|'.'
name|'shuffle'
op|'('
name|'assign_parts_list'
op|')'
newline|'\n'
comment|'# reset assign_parts map for next iteration'
nl|'\n'
name|'assign_parts'
op|'='
name|'defaultdict'
op|'('
name|'list'
op|')'
newline|'\n'
nl|'\n'
name|'num_part_replicas'
op|'='
name|'sum'
op|'('
name|'len'
op|'('
name|'r'
op|')'
name|'for'
name|'p'
op|','
name|'r'
name|'in'
name|'assign_parts_list'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
string|'"Gathered %d parts"'
op|','
name|'num_part_replicas'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_reassign_parts'
op|'('
name|'assign_parts_list'
op|','
name|'replica_plan'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
string|'"Assigned %d parts"'
op|','
name|'num_part_replicas'
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'not'
name|'sum'
op|'('
name|'d'
op|'['
string|"'parts_wanted'"
op|']'
op|'<'
number|'0'
name|'for'
name|'d'
name|'in'
nl|'\n'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'finish_status'
op|'='
string|"'Finished'"
newline|'\n'
name|'break'
newline|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'finish_status'
op|'='
string|"'Unable to finish'"
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
string|"'%s rebalance plan after %s attempts'"
op|'%'
op|'('
nl|'\n'
name|'finish_status'
op|','
name|'gather_count'
op|'+'
number|'1'
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'devs_changed'
op|'='
name|'False'
newline|'\n'
name|'self'
op|'.'
name|'version'
op|'+='
number|'1'
newline|'\n'
name|'changed_parts'
op|'='
name|'self'
op|'.'
name|'_build_dispersion_graph'
op|'('
name|'old_replica2part2dev'
op|')'
newline|'\n'
nl|'\n'
comment|'# clean up the cache'
nl|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'dev'
op|'.'
name|'pop'
op|'('
string|"'tiers'"
op|','
name|'None'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'return'
name|'changed_parts'
op|','
name|'self'
op|'.'
name|'get_balance'
op|'('
op|')'
op|','
name|'removed_devs'
newline|'\n'
nl|'\n'
DECL|member|_build_dispersion_graph
dedent|''
name|'def'
name|'_build_dispersion_graph'
op|'('
name|'self'
op|','
name|'old_replica2part2dev'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Build a dict of all tiers in the cluster to a list of the number of\n        parts with a replica count at each index.  The values of the dict will\n        be lists of length the maximum whole replica + 1 so that the\n        graph[tier][3] is the number of parts with in the tier with 3 replicas\n        and graph [tier][0] is the number of parts not assigned in this tier.\n\n        i.e.\n        {\n            <tier>: [\n                <number_of_parts_with_0_replicas>,\n                <number_of_parts_with_1_replicas>,\n                ...\n                <number_of_parts_with_n_replicas>,\n                ],\n            ...\n        }\n\n        :param old_replica2part2dev: if called from rebalance, the\n            old_replica2part2dev can be used to count moved moved parts.\n\n        :returns: number of parts with different assignments than\n            old_replica2part2dev if provided\n        """'
newline|'\n'
nl|'\n'
comment|"# Since we're going to loop over every replica of every part we'll"
nl|'\n'
comment|'# also count up changed_parts if old_replica2part2dev is passed in'
nl|'\n'
name|'old_replica2part2dev'
op|'='
name|'old_replica2part2dev'
name|'or'
op|'['
op|']'
newline|'\n'
comment|'# Compare the partition allocation before and after the rebalance'
nl|'\n'
comment|'# Only changed device ids are taken into account; devices might be'
nl|'\n'
comment|'# "touched" during the rebalance, but actually not really moved'
nl|'\n'
name|'changed_parts'
op|'='
number|'0'
newline|'\n'
nl|'\n'
name|'int_replicas'
op|'='
name|'int'
op|'('
name|'math'
op|'.'
name|'ceil'
op|'('
name|'self'
op|'.'
name|'replicas'
op|')'
op|')'
newline|'\n'
name|'max_allowed_replicas'
op|'='
name|'self'
op|'.'
name|'_build_max_replicas_by_tier'
op|'('
op|')'
newline|'\n'
name|'parts_at_risk'
op|'='
number|'0'
newline|'\n'
nl|'\n'
name|'dispersion_graph'
op|'='
op|'{'
op|'}'
newline|'\n'
comment|'# go over all the devices holding each replica part by part'
nl|'\n'
name|'for'
name|'part_id'
op|','
name|'dev_ids'
name|'in'
name|'enumerate'
op|'('
nl|'\n'
name|'six'
op|'.'
name|'moves'
op|'.'
name|'zip'
op|'('
op|'*'
name|'self'
op|'.'
name|'_replica2part2dev'
op|')'
op|')'
op|':'
newline|'\n'
comment|'# count the number of replicas of this part for each tier of each'
nl|'\n'
comment|'# device, some devices may have overlapping tiers!'
nl|'\n'
indent|'            '
name|'replicas_at_tier'
op|'='
name|'defaultdict'
op|'('
name|'int'
op|')'
newline|'\n'
name|'for'
name|'rep_id'
op|','
name|'dev'
name|'in'
name|'enumerate'
op|'('
name|'iter'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'devs'
op|'['
name|'dev_id'
op|']'
name|'for'
name|'dev_id'
name|'in'
name|'dev_ids'
op|')'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'for'
name|'tier'
name|'in'
op|'('
name|'dev'
op|'.'
name|'get'
op|'('
string|"'tiers'"
op|')'
name|'or'
name|'tiers_for_dev'
op|'('
name|'dev'
op|')'
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'replicas_at_tier'
op|'['
name|'tier'
op|']'
op|'+='
number|'1'
newline|'\n'
comment|'# IndexErrors will be raised if the replicas are increased or'
nl|'\n'
comment|'# decreased, and that actually means the partition has changed'
nl|'\n'
dedent|''
name|'try'
op|':'
newline|'\n'
indent|'                    '
name|'old_device'
op|'='
name|'old_replica2part2dev'
op|'['
name|'rep_id'
op|']'
op|'['
name|'part_id'
op|']'
newline|'\n'
dedent|''
name|'except'
name|'IndexError'
op|':'
newline|'\n'
indent|'                    '
name|'changed_parts'
op|'+='
number|'1'
newline|'\n'
name|'continue'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'old_device'
op|'!='
name|'dev'
op|'['
string|"'id'"
op|']'
op|':'
newline|'\n'
indent|'                    '
name|'changed_parts'
op|'+='
number|'1'
newline|'\n'
dedent|''
dedent|''
name|'part_at_risk'
op|'='
name|'False'
newline|'\n'
comment|"# update running totals for each tiers' number of parts with a"
nl|'\n'
comment|'# given replica count'
nl|'\n'
name|'for'
name|'tier'
op|','
name|'replicas'
name|'in'
name|'replicas_at_tier'
op|'.'
name|'items'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'tier'
name|'not'
name|'in'
name|'dispersion_graph'
op|':'
newline|'\n'
indent|'                    '
name|'dispersion_graph'
op|'['
name|'tier'
op|']'
op|'='
op|'['
name|'self'
op|'.'
name|'parts'
op|']'
op|'+'
op|'['
number|'0'
op|']'
op|'*'
name|'int_replicas'
newline|'\n'
dedent|''
name|'dispersion_graph'
op|'['
name|'tier'
op|']'
op|'['
number|'0'
op|']'
op|'-='
number|'1'
newline|'\n'
name|'dispersion_graph'
op|'['
name|'tier'
op|']'
op|'['
name|'replicas'
op|']'
op|'+='
number|'1'
newline|'\n'
name|'if'
name|'replicas'
op|'>'
name|'max_allowed_replicas'
op|'['
name|'tier'
op|']'
op|':'
newline|'\n'
indent|'                    '
name|'part_at_risk'
op|'='
name|'True'
newline|'\n'
comment|'# this part may be at risk in multiple tiers, but we only count it'
nl|'\n'
comment|'# as at_risk once'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'part_at_risk'
op|':'
newline|'\n'
indent|'                '
name|'parts_at_risk'
op|'+='
number|'1'
newline|'\n'
dedent|''
dedent|''
name|'self'
op|'.'
name|'_dispersion_graph'
op|'='
name|'dispersion_graph'
newline|'\n'
name|'self'
op|'.'
name|'dispersion'
op|'='
number|'100.0'
op|'*'
name|'parts_at_risk'
op|'/'
name|'self'
op|'.'
name|'parts'
newline|'\n'
name|'return'
name|'changed_parts'
newline|'\n'
nl|'\n'
DECL|member|validate
dedent|''
name|'def'
name|'validate'
op|'('
name|'self'
op|','
name|'stats'
op|'='
name|'False'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Validate the ring.\n\n        This is a safety function to try to catch any bugs in the building\n        process. It ensures partitions have been assigned to real devices,\n        aren\'t doubly assigned, etc. It can also optionally check the even\n        distribution of partitions across devices.\n\n        :param stats: if True, check distribution of partitions across devices\n        :returns: if stats is True, a tuple of (device_usage, worst_stat), else\n                  (None, None). device_usage[dev_id] will equal the number of\n                  partitions assigned to that device. worst_stat will equal the\n                  number of partitions the worst device is skewed from the\n                  number it should have.\n        :raises RingValidationError: problem was found with the ring.\n        """'
newline|'\n'
nl|'\n'
comment|'# "len" showed up in profiling, so it\'s just computed once.'
nl|'\n'
name|'dev_len'
op|'='
name|'len'
op|'('
name|'self'
op|'.'
name|'devs'
op|')'
newline|'\n'
nl|'\n'
name|'parts_on_devs'
op|'='
name|'sum'
op|'('
name|'d'
op|'['
string|"'parts'"
op|']'
name|'for'
name|'d'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'_replica2part2dev'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|"'_replica2part2dev empty; did you forget to rebalance?'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'parts_in_map'
op|'='
name|'sum'
op|'('
name|'len'
op|'('
name|'p2d'
op|')'
name|'for'
name|'p2d'
name|'in'
name|'self'
op|'.'
name|'_replica2part2dev'
op|')'
newline|'\n'
name|'if'
name|'parts_on_devs'
op|'!='
name|'parts_in_map'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|"'All partitions are not double accounted for: %d != %d'"
op|'%'
nl|'\n'
op|'('
name|'parts_on_devs'
op|','
name|'parts_in_map'
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'stats'
op|':'
newline|'\n'
comment|'# dev_usage[dev_id] will equal the number of partitions assigned to'
nl|'\n'
comment|'# that device.'
nl|'\n'
indent|'            '
name|'dev_usage'
op|'='
name|'array'
op|'('
string|"'I'"
op|','
op|'('
number|'0'
name|'for'
name|'_junk'
name|'in'
name|'range'
op|'('
name|'dev_len'
op|')'
op|')'
op|')'
newline|'\n'
name|'for'
name|'part2dev'
name|'in'
name|'self'
op|'.'
name|'_replica2part2dev'
op|':'
newline|'\n'
indent|'                '
name|'for'
name|'dev_id'
name|'in'
name|'part2dev'
op|':'
newline|'\n'
indent|'                    '
name|'dev_usage'
op|'['
name|'dev_id'
op|']'
op|'+='
number|'1'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
dedent|''
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'not'
name|'isinstance'
op|'('
name|'dev'
op|'['
string|"'port'"
op|']'
op|','
name|'int'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|'"Device %d has port %r, which is not an integer."'
op|'%'
nl|'\n'
op|'('
name|'dev'
op|'['
string|"'id'"
op|']'
op|','
name|'dev'
op|'['
string|"'port'"
op|']'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'int_replicas'
op|'='
name|'int'
op|'('
name|'math'
op|'.'
name|'ceil'
op|'('
name|'self'
op|'.'
name|'replicas'
op|')'
op|')'
newline|'\n'
name|'rep2part_len'
op|'='
name|'map'
op|'('
name|'len'
op|','
name|'self'
op|'.'
name|'_replica2part2dev'
op|')'
newline|'\n'
comment|"# check the assignments of each part's replicas"
nl|'\n'
name|'for'
name|'part'
name|'in'
name|'range'
op|'('
name|'self'
op|'.'
name|'parts'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'devs_for_part'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'replica'
op|','
name|'part_len'
name|'in'
name|'enumerate'
op|'('
name|'rep2part_len'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'part_len'
op|'<='
name|'part'
op|':'
newline|'\n'
comment|'# last replica may be short on parts because of floating'
nl|'\n'
comment|'# replica count'
nl|'\n'
indent|'                    '
name|'if'
name|'replica'
op|'+'
number|'1'
op|'<'
name|'int_replicas'
op|':'
newline|'\n'
indent|'                        '
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|'"The partition assignments of replica %r were "'
nl|'\n'
string|'"shorter than expected (%s < %s) - this should "'
nl|'\n'
string|'"only happen for the last replica"'
op|'%'
op|'('
nl|'\n'
name|'replica'
op|','
nl|'\n'
name|'len'
op|'('
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
op|')'
op|','
nl|'\n'
name|'self'
op|'.'
name|'parts'
op|','
nl|'\n'
op|')'
op|')'
newline|'\n'
dedent|''
name|'break'
newline|'\n'
dedent|''
name|'dev_id'
op|'='
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
op|'['
name|'part'
op|']'
newline|'\n'
name|'if'
name|'dev_id'
op|'>='
name|'dev_len'
name|'or'
name|'not'
name|'self'
op|'.'
name|'devs'
op|'['
name|'dev_id'
op|']'
op|':'
newline|'\n'
indent|'                    '
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|'"Partition %d, replica %d was not allocated "'
nl|'\n'
string|'"to a device."'
op|'%'
nl|'\n'
op|'('
name|'part'
op|','
name|'replica'
op|')'
op|')'
newline|'\n'
dedent|''
name|'devs_for_part'
op|'.'
name|'append'
op|'('
name|'dev_id'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'len'
op|'('
name|'devs_for_part'
op|')'
op|'!='
name|'len'
op|'('
name|'set'
op|'('
name|'devs_for_part'
op|')'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|'"The partition %s has been assigned to "'
nl|'\n'
string|'"duplicate devices %r"'
op|'%'
op|'('
nl|'\n'
name|'part'
op|','
name|'devs_for_part'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'stats'
op|':'
newline|'\n'
indent|'            '
name|'weight_of_one_part'
op|'='
name|'self'
op|'.'
name|'weight_of_one_part'
op|'('
op|')'
newline|'\n'
name|'worst'
op|'='
number|'0'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'not'
name|'dev'
op|'['
string|"'weight'"
op|']'
op|':'
newline|'\n'
indent|'                    '
name|'if'
name|'dev_usage'
op|'['
name|'dev'
op|'['
string|"'id'"
op|']'
op|']'
op|':'
newline|'\n'
comment|'# If a device has no weight, but has partitions, then'
nl|'\n'
comment|'# its overage is considered "infinity" and therefore'
nl|'\n'
comment|'# always the worst possible. We show MAX_BALANCE for'
nl|'\n'
comment|'# convenience.'
nl|'\n'
indent|'                        '
name|'worst'
op|'='
name|'MAX_BALANCE'
newline|'\n'
name|'break'
newline|'\n'
dedent|''
name|'continue'
newline|'\n'
dedent|''
name|'skew'
op|'='
name|'abs'
op|'('
number|'100.0'
op|'*'
name|'dev_usage'
op|'['
name|'dev'
op|'['
string|"'id'"
op|']'
op|']'
op|'/'
nl|'\n'
op|'('
name|'dev'
op|'['
string|"'weight'"
op|']'
op|'*'
name|'weight_of_one_part'
op|')'
op|'-'
number|'100.0'
op|')'
newline|'\n'
name|'if'
name|'skew'
op|'>'
name|'worst'
op|':'
newline|'\n'
indent|'                    '
name|'worst'
op|'='
name|'skew'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'dev_usage'
op|','
name|'worst'
newline|'\n'
dedent|''
name|'return'
name|'None'
op|','
name|'None'
newline|'\n'
nl|'\n'
DECL|member|_build_balance_per_dev
dedent|''
name|'def'
name|'_build_balance_per_dev'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Build a map of <device_id> => <balance> where <balance> is a float\n        representing the percentage difference from the desired amount of\n        partitions a given device wants and the amount it has.\n\n        N.B. this method only considers a device\'s weight and the parts\n        assigned, not the parts wanted according to the replica plan.\n        """'
newline|'\n'
name|'weight_of_one_part'
op|'='
name|'self'
op|'.'
name|'weight_of_one_part'
op|'('
op|')'
newline|'\n'
name|'balance_per_dev'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'not'
name|'dev'
op|'['
string|"'weight'"
op|']'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'dev'
op|'['
string|"'parts'"
op|']'
op|':'
newline|'\n'
comment|'# If a device has no weight, but has partitions, then its'
nl|'\n'
comment|'# overage is considered "infinity" and therefore always the'
nl|'\n'
comment|'# worst possible. We show MAX_BALANCE for convenience.'
nl|'\n'
indent|'                    '
name|'balance'
op|'='
name|'MAX_BALANCE'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                    '
name|'balance'
op|'='
number|'0'
newline|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'balance'
op|'='
number|'100.0'
op|'*'
name|'dev'
op|'['
string|"'parts'"
op|']'
op|'/'
op|'('
nl|'\n'
name|'dev'
op|'['
string|"'weight'"
op|']'
op|'*'
name|'weight_of_one_part'
op|')'
op|'-'
number|'100.0'
newline|'\n'
dedent|''
name|'balance_per_dev'
op|'['
name|'dev'
op|'['
string|"'id'"
op|']'
op|']'
op|'='
name|'balance'
newline|'\n'
dedent|''
name|'return'
name|'balance_per_dev'
newline|'\n'
nl|'\n'
DECL|member|get_balance
dedent|''
name|'def'
name|'get_balance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Get the balance of the ring. The balance value is the highest\n        percentage off the desired amount of partitions a given device\n        wants. For instance, if the "worst" device wants (based on its\n        weight relative to the sum of all the devices\' weights) 123\n        partitions and it has 124 partitions, the balance value would\n        be 0.83 (1 extra / 123 wanted * 100 for percentage).\n\n        :returns: balance of the ring\n        """'
newline|'\n'
name|'balance_per_dev'
op|'='
name|'self'
op|'.'
name|'_build_balance_per_dev'
op|'('
op|')'
newline|'\n'
name|'return'
name|'max'
op|'('
name|'abs'
op|'('
name|'b'
op|')'
name|'for'
name|'b'
name|'in'
name|'balance_per_dev'
op|'.'
name|'values'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|get_required_overload
dedent|''
name|'def'
name|'get_required_overload'
op|'('
name|'self'
op|','
name|'weighted'
op|'='
name|'None'
op|','
name|'wanted'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Returns the minimum overload value required to make the ring maximally\n        dispersed.\n\n        The required overload is the largest percentage change of any single\n        device from its weighted replicanth to its wanted replicanth (note\n        under weighted devices have a negative percentage change) to archive\n        dispersion - that is to say a single device that must be overloaded by\n        5% is worse than 5 devices in a single tier overloaded by 1%.\n        """'
newline|'\n'
name|'weighted'
op|'='
name|'weighted'
name|'or'
name|'self'
op|'.'
name|'_build_weighted_replicas_by_tier'
op|'('
op|')'
newline|'\n'
name|'wanted'
op|'='
name|'wanted'
name|'or'
name|'self'
op|'.'
name|'_build_wanted_replicas_by_tier'
op|'('
op|')'
newline|'\n'
name|'max_overload'
op|'='
number|'0.0'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'tier'
op|'='
op|'('
name|'dev'
op|'['
string|"'region'"
op|']'
op|','
name|'dev'
op|'['
string|"'zone'"
op|']'
op|','
name|'dev'
op|'['
string|"'ip'"
op|']'
op|','
name|'dev'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'dev'
op|'['
string|"'weight'"
op|']'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'tier'
name|'not'
name|'in'
name|'wanted'
name|'or'
name|'not'
name|'wanted'
op|'['
name|'tier'
op|']'
op|':'
newline|'\n'
indent|'                    '
name|'continue'
newline|'\n'
dedent|''
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|"'Device %s has zero weight and '"
nl|'\n'
string|"'should not want any replicas'"
op|'%'
op|'('
name|'tier'
op|','
op|')'
op|')'
newline|'\n'
dedent|''
name|'required'
op|'='
op|'('
name|'wanted'
op|'['
name|'tier'
op|']'
op|'-'
name|'weighted'
op|'['
name|'tier'
op|']'
op|')'
op|'/'
name|'weighted'
op|'['
name|'tier'
op|']'
newline|'\n'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
string|"'%s wants %s and is weighted for %s so '"
nl|'\n'
string|"'therefore requires %s overload'"
op|'%'
op|'('
nl|'\n'
name|'tier'
op|','
name|'wanted'
op|'['
name|'tier'
op|']'
op|','
name|'weighted'
op|'['
name|'tier'
op|']'
op|','
nl|'\n'
name|'required'
op|')'
op|')'
newline|'\n'
name|'if'
name|'required'
op|'>'
name|'max_overload'
op|':'
newline|'\n'
indent|'                '
name|'max_overload'
op|'='
name|'required'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'max_overload'
newline|'\n'
nl|'\n'
DECL|member|pretend_min_part_hours_passed
dedent|''
name|'def'
name|'pretend_min_part_hours_passed'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Override min_part_hours by marking all partitions as having been moved\n        255 hours ago and last move epoch to \'the beginning of time\'. This can\n        be used to force a full rebalance on the next call to rebalance.\n        """'
newline|'\n'
name|'for'
name|'part'
name|'in'
name|'range'
op|'('
name|'self'
op|'.'
name|'parts'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'='
number|'0xff'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'_last_part_moves_epoch'
op|'='
number|'0'
newline|'\n'
nl|'\n'
DECL|member|get_part_devices
dedent|''
name|'def'
name|'get_part_devices'
op|'('
name|'self'
op|','
name|'part'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Get the devices that are responsible for the partition,\n        filtering out duplicates.\n\n        :param part: partition to get devices for\n        :returns: list of device dicts\n        """'
newline|'\n'
name|'devices'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_devs_for_part'
op|'('
name|'part'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'dev'
name|'not'
name|'in'
name|'devices'
op|':'
newline|'\n'
indent|'                '
name|'devices'
op|'.'
name|'append'
op|'('
name|'dev'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'devices'
newline|'\n'
nl|'\n'
DECL|member|_iter_devs
dedent|''
name|'def'
name|'_iter_devs'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Returns an iterator all the non-None devices in the ring. Note that\n        this means list(b._iter_devs())[some_id] may not equal b.devs[some_id];\n        you will have to check the \'id\' key of each device to obtain its\n        dev_id.\n        """'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'devs'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'dev'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'                '
name|'yield'
name|'dev'
newline|'\n'
nl|'\n'
DECL|member|_build_tier2children
dedent|''
dedent|''
dedent|''
name|'def'
name|'_build_tier2children'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Wrap helper build_tier_tree so exclude zero-weight devices.\n        """'
newline|'\n'
name|'return'
name|'build_tier_tree'
op|'('
name|'d'
name|'for'
name|'d'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
name|'if'
name|'d'
op|'['
string|"'weight'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_set_parts_wanted
dedent|''
name|'def'
name|'_set_parts_wanted'
op|'('
name|'self'
op|','
name|'replica_plan'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Sets the parts_wanted key for each of the devices to the number of\n        partitions the device wants based on its relative weight. This key is\n        used to sort the devices according to "most wanted" during rebalancing\n        to best distribute partitions. A negative parts_wanted indicates the\n        device is "overweight" and wishes to give partitions away if possible.\n\n        :param replica_plan: a dict of dicts, as returned from\n                             _build_replica_plan, that that maps\n                             each tier to it\'s target replicanths.\n        """'
newline|'\n'
name|'tier2children'
op|'='
name|'self'
op|'.'
name|'_build_tier2children'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'parts_by_tier'
op|'='
name|'defaultdict'
op|'('
name|'int'
op|')'
newline|'\n'
nl|'\n'
DECL|function|place_parts
name|'def'
name|'place_parts'
op|'('
name|'tier'
op|','
name|'parts'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'parts_by_tier'
op|'['
name|'tier'
op|']'
op|'='
name|'parts'
newline|'\n'
name|'sub_tiers'
op|'='
name|'sorted'
op|'('
name|'tier2children'
op|'['
name|'tier'
op|']'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'sub_tiers'
op|':'
newline|'\n'
indent|'                '
name|'return'
newline|'\n'
dedent|''
name|'to_place'
op|'='
name|'defaultdict'
op|'('
name|'int'
op|')'
newline|'\n'
name|'for'
name|'t'
name|'in'
name|'sub_tiers'
op|':'
newline|'\n'
indent|'                '
name|'to_place'
op|'['
name|'t'
op|']'
op|'='
name|'int'
op|'('
name|'math'
op|'.'
name|'floor'
op|'('
nl|'\n'
name|'replica_plan'
op|'['
name|'t'
op|']'
op|'['
string|"'target'"
op|']'
op|'*'
name|'self'
op|'.'
name|'parts'
op|')'
op|')'
newline|'\n'
name|'parts'
op|'-='
name|'to_place'
op|'['
name|'t'
op|']'
newline|'\n'
nl|'\n'
comment|"# if there's some parts left over, just throw 'em about"
nl|'\n'
dedent|''
name|'sub_tier_gen'
op|'='
name|'itertools'
op|'.'
name|'cycle'
op|'('
name|'sorted'
op|'('
nl|'\n'
name|'sub_tiers'
op|','
name|'key'
op|'='
name|'lambda'
name|'t'
op|':'
name|'replica_plan'
op|'['
name|'t'
op|']'
op|'['
string|"'target'"
op|']'
op|')'
op|')'
newline|'\n'
name|'while'
name|'parts'
op|':'
newline|'\n'
indent|'                '
name|'t'
op|'='
name|'next'
op|'('
name|'sub_tier_gen'
op|')'
newline|'\n'
name|'to_place'
op|'['
name|'t'
op|']'
op|'+='
number|'1'
newline|'\n'
name|'parts'
op|'-='
number|'1'
newline|'\n'
nl|'\n'
dedent|''
name|'for'
name|'t'
op|','
name|'p'
name|'in'
name|'to_place'
op|'.'
name|'items'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                '
name|'place_parts'
op|'('
name|'t'
op|','
name|'p'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'total_parts'
op|'='
name|'int'
op|'('
name|'self'
op|'.'
name|'replicas'
op|'*'
name|'self'
op|'.'
name|'parts'
op|')'
newline|'\n'
name|'place_parts'
op|'('
op|'('
op|')'
op|','
name|'total_parts'
op|')'
newline|'\n'
nl|'\n'
comment|'# belts & suspenders/paranoia -  at every level, the sum of'
nl|'\n'
comment|'# parts_by_tier should be total_parts for the ring'
nl|'\n'
name|'tiers'
op|'='
op|'['
string|"'cluster'"
op|','
string|"'regions'"
op|','
string|"'zones'"
op|','
string|"'servers'"
op|','
string|"'devices'"
op|']'
newline|'\n'
name|'for'
name|'i'
op|','
name|'tier_name'
name|'in'
name|'enumerate'
op|'('
name|'tiers'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'parts_at_tier'
op|'='
name|'sum'
op|'('
name|'parts_by_tier'
op|'['
name|'t'
op|']'
name|'for'
name|'t'
name|'in'
name|'parts_by_tier'
nl|'\n'
name|'if'
name|'len'
op|'('
name|'t'
op|')'
op|'=='
name|'i'
op|')'
newline|'\n'
name|'if'
name|'parts_at_tier'
op|'!='
name|'total_parts'
op|':'
newline|'\n'
indent|'                '
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|"'%s != %s at tier %s'"
op|'%'
op|'('
nl|'\n'
name|'parts_at_tier'
op|','
name|'total_parts'
op|','
name|'tier_name'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'not'
name|'dev'
op|'['
string|"'weight'"
op|']'
op|':'
newline|'\n'
comment|'# With no weight, that means we wish to "drain" the device. So'
nl|'\n'
comment|'# we set the parts_wanted to a really large negative number to'
nl|'\n'
comment|'# indicate its strong desire to give up everything it has.'
nl|'\n'
indent|'                '
name|'dev'
op|'['
string|"'parts_wanted'"
op|']'
op|'='
op|'-'
name|'self'
op|'.'
name|'parts'
op|'*'
name|'self'
op|'.'
name|'replicas'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'tier'
op|'='
op|'('
name|'dev'
op|'['
string|"'region'"
op|']'
op|','
name|'dev'
op|'['
string|"'zone'"
op|']'
op|','
name|'dev'
op|'['
string|"'ip'"
op|']'
op|','
name|'dev'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
name|'dev'
op|'['
string|"'parts_wanted'"
op|']'
op|'='
name|'parts_by_tier'
op|'['
name|'tier'
op|']'
op|'-'
name|'dev'
op|'['
string|"'parts'"
op|']'
newline|'\n'
nl|'\n'
DECL|member|_update_last_part_moves
dedent|''
dedent|''
dedent|''
name|'def'
name|'_update_last_part_moves'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Updates how many hours ago each partition was moved based on the\n        current time. The builder won\'t move a partition that has been moved\n        more recently than min_part_hours.\n        """'
newline|'\n'
name|'elapsed_hours'
op|'='
name|'int'
op|'('
name|'time'
op|'('
op|')'
op|'-'
name|'self'
op|'.'
name|'_last_part_moves_epoch'
op|')'
op|'/'
number|'3600'
newline|'\n'
name|'if'
name|'elapsed_hours'
op|'<='
number|'0'
op|':'
newline|'\n'
indent|'            '
name|'return'
newline|'\n'
dedent|''
name|'for'
name|'part'
name|'in'
name|'range'
op|'('
name|'self'
op|'.'
name|'parts'
op|')'
op|':'
newline|'\n'
comment|'# The "min(self._last_part_moves[part] + elapsed_hours, 0xff)"'
nl|'\n'
comment|'# which was here showed up in profiling, so it got inlined.'
nl|'\n'
indent|'            '
name|'last_plus_elapsed'
op|'='
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'+'
name|'elapsed_hours'
newline|'\n'
name|'if'
name|'last_plus_elapsed'
op|'<'
number|'0xff'
op|':'
newline|'\n'
indent|'                '
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'='
name|'last_plus_elapsed'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'='
number|'0xff'
newline|'\n'
dedent|''
dedent|''
name|'self'
op|'.'
name|'_last_part_moves_epoch'
op|'='
name|'int'
op|'('
name|'time'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_gather_parts_from_failed_devices
dedent|''
name|'def'
name|'_gather_parts_from_failed_devices'
op|'('
name|'self'
op|','
name|'assign_parts'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Update the map of partition => [replicas] to be reassigned from\n        removed devices.\n        """'
newline|'\n'
comment|'# First we gather partitions from removed devices. Since removed'
nl|'\n'
comment|'# devices usually indicate device failures, we have no choice but to'
nl|'\n'
comment|'# reassign these partitions. However, we mark them as moved so later'
nl|'\n'
comment|'# choices will skip other replicas of the same partition if possible.'
nl|'\n'
nl|'\n'
name|'if'
name|'self'
op|'.'
name|'_remove_devs'
op|':'
newline|'\n'
indent|'            '
name|'dev_ids'
op|'='
op|'['
name|'d'
op|'['
string|"'id'"
op|']'
name|'for'
name|'d'
name|'in'
name|'self'
op|'.'
name|'_remove_devs'
name|'if'
name|'d'
op|'['
string|"'parts'"
op|']'
op|']'
newline|'\n'
name|'if'
name|'dev_ids'
op|':'
newline|'\n'
indent|'                '
name|'for'
name|'part'
op|','
name|'replica'
name|'in'
name|'self'
op|'.'
name|'_each_part_replica'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'dev_id'
op|'='
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
op|'['
name|'part'
op|']'
newline|'\n'
name|'if'
name|'dev_id'
name|'in'
name|'dev_ids'
op|':'
newline|'\n'
indent|'                        '
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
op|'['
name|'part'
op|']'
op|'='
name|'NONE_DEV'
newline|'\n'
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'='
number|'0'
newline|'\n'
name|'assign_parts'
op|'['
name|'part'
op|']'
op|'.'
name|'append'
op|'('
name|'replica'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
nl|'\n'
string|'"Gathered %d/%d from dev %d [dev removed]"'
op|','
nl|'\n'
name|'part'
op|','
name|'replica'
op|','
name|'dev_id'
op|')'
newline|'\n'
dedent|''
dedent|''
dedent|''
dedent|''
name|'removed_devs'
op|'='
number|'0'
newline|'\n'
name|'while'
name|'self'
op|'.'
name|'_remove_devs'
op|':'
newline|'\n'
indent|'            '
name|'remove_dev_id'
op|'='
name|'self'
op|'.'
name|'_remove_devs'
op|'.'
name|'pop'
op|'('
op|')'
op|'['
string|"'id'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
string|'"Removing dev %d"'
op|','
name|'remove_dev_id'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'devs'
op|'['
name|'remove_dev_id'
op|']'
op|'='
name|'None'
newline|'\n'
name|'removed_devs'
op|'+='
number|'1'
newline|'\n'
dedent|''
name|'return'
name|'removed_devs'
newline|'\n'
nl|'\n'
DECL|member|_adjust_replica2part2dev_size
dedent|''
name|'def'
name|'_adjust_replica2part2dev_size'
op|'('
name|'self'
op|','
name|'to_assign'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Make sure that the lengths of the arrays in _replica2part2dev\n        are correct for the current value of self.replicas.\n\n        Example:\n        self.part_power = 8\n        self.replicas = 2.25\n\n        self._replica2part2dev will contain 3 arrays: the first 2 of\n        length 256 (2**8), and the last of length 64 (0.25 * 2**8).\n\n        Update the mapping of partition => [replicas] that need assignment.\n        """'
newline|'\n'
name|'fractional_replicas'
op|','
name|'whole_replicas'
op|'='
name|'math'
op|'.'
name|'modf'
op|'('
name|'self'
op|'.'
name|'replicas'
op|')'
newline|'\n'
name|'whole_replicas'
op|'='
name|'int'
op|'('
name|'whole_replicas'
op|')'
newline|'\n'
name|'removed_parts'
op|'='
number|'0'
newline|'\n'
name|'new_parts'
op|'='
number|'0'
newline|'\n'
nl|'\n'
name|'desired_lengths'
op|'='
op|'['
name|'self'
op|'.'
name|'parts'
op|']'
op|'*'
name|'whole_replicas'
newline|'\n'
name|'if'
name|'fractional_replicas'
op|':'
newline|'\n'
indent|'            '
name|'desired_lengths'
op|'.'
name|'append'
op|'('
name|'int'
op|'('
name|'self'
op|'.'
name|'parts'
op|'*'
name|'fractional_replicas'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'self'
op|'.'
name|'_replica2part2dev'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
comment|'# If we crossed an integer threshold (say, 4.1 --> 4),'
nl|'\n'
comment|"# we'll have a partial extra replica clinging on here. Clean"
nl|'\n'
comment|'# up any such extra stuff.'
nl|'\n'
indent|'            '
name|'for'
name|'part2dev'
name|'in'
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'len'
op|'('
name|'desired_lengths'
op|')'
op|':'
op|']'
op|':'
newline|'\n'
indent|'                '
name|'for'
name|'dev_id'
name|'in'
name|'part2dev'
op|':'
newline|'\n'
indent|'                    '
name|'dev_losing_part'
op|'='
name|'self'
op|'.'
name|'devs'
op|'['
name|'dev_id'
op|']'
newline|'\n'
name|'dev_losing_part'
op|'['
string|"'parts'"
op|']'
op|'-='
number|'1'
newline|'\n'
name|'removed_parts'
op|'-='
number|'1'
newline|'\n'
dedent|''
dedent|''
name|'self'
op|'.'
name|'_replica2part2dev'
op|'='
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
op|':'
name|'len'
op|'('
name|'desired_lengths'
op|')'
op|']'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'_replica2part2dev'
op|'='
op|'['
op|']'
newline|'\n'
nl|'\n'
dedent|''
name|'for'
name|'replica'
op|','
name|'desired_length'
name|'in'
name|'enumerate'
op|'('
name|'desired_lengths'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'replica'
op|'<'
name|'len'
op|'('
name|'self'
op|'.'
name|'_replica2part2dev'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'part2dev'
op|'='
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
newline|'\n'
name|'if'
name|'len'
op|'('
name|'part2dev'
op|')'
op|'<'
name|'desired_length'
op|':'
newline|'\n'
comment|'# Not long enough: needs to be extended and the'
nl|'\n'
comment|'# newly-added pieces assigned to devices.'
nl|'\n'
indent|'                    '
name|'for'
name|'part'
name|'in'
name|'range'
op|'('
name|'len'
op|'('
name|'part2dev'
op|')'
op|','
name|'desired_length'
op|')'
op|':'
newline|'\n'
indent|'                        '
name|'to_assign'
op|'['
name|'part'
op|']'
op|'.'
name|'append'
op|'('
name|'replica'
op|')'
newline|'\n'
name|'part2dev'
op|'.'
name|'append'
op|'('
name|'NONE_DEV'
op|')'
newline|'\n'
name|'new_parts'
op|'+='
number|'1'
newline|'\n'
dedent|''
dedent|''
name|'elif'
name|'len'
op|'('
name|'part2dev'
op|')'
op|'>'
name|'desired_length'
op|':'
newline|'\n'
comment|'# Too long: truncate this mapping.'
nl|'\n'
indent|'                    '
name|'for'
name|'part'
name|'in'
name|'range'
op|'('
name|'desired_length'
op|','
name|'len'
op|'('
name|'part2dev'
op|')'
op|')'
op|':'
newline|'\n'
indent|'                        '
name|'dev_losing_part'
op|'='
name|'self'
op|'.'
name|'devs'
op|'['
name|'part2dev'
op|'['
name|'part'
op|']'
op|']'
newline|'\n'
name|'dev_losing_part'
op|'['
string|"'parts'"
op|']'
op|'-='
number|'1'
newline|'\n'
name|'removed_parts'
op|'-='
number|'1'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
op|'='
name|'part2dev'
op|'['
op|':'
name|'desired_length'
op|']'
newline|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
comment|'# Mapping not present at all: make one up and assign'
nl|'\n'
comment|'# all of it.'
nl|'\n'
indent|'                '
name|'for'
name|'part'
name|'in'
name|'range'
op|'('
name|'desired_length'
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'to_assign'
op|'['
name|'part'
op|']'
op|'.'
name|'append'
op|'('
name|'replica'
op|')'
newline|'\n'
name|'new_parts'
op|'+='
number|'1'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'_replica2part2dev'
op|'.'
name|'append'
op|'('
nl|'\n'
name|'array'
op|'('
string|"'H'"
op|','
name|'itertools'
op|'.'
name|'repeat'
op|'('
name|'NONE_DEV'
op|','
name|'desired_length'
op|')'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
nl|'\n'
string|'"%d new parts and %d removed parts from replica-count change"'
op|','
nl|'\n'
name|'new_parts'
op|','
name|'removed_parts'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_gather_parts_for_dispersion
dedent|''
name|'def'
name|'_gather_parts_for_dispersion'
op|'('
name|'self'
op|','
name|'assign_parts'
op|','
name|'replica_plan'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Update the map of partition => [replicas] to be reassigned from\n        insufficiently-far-apart replicas.\n        """'
newline|'\n'
comment|'# Now we gather partitions that are "at risk" because they aren\'t'
nl|'\n'
comment|'# currently sufficient spread out across the cluster.'
nl|'\n'
name|'for'
name|'part'
name|'in'
name|'range'
op|'('
name|'self'
op|'.'
name|'parts'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'<'
name|'self'
op|'.'
name|'min_part_hours'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
comment|'# First, add up the count of replicas at each tier for each'
nl|'\n'
comment|'# partition.'
nl|'\n'
dedent|''
name|'replicas_at_tier'
op|'='
name|'defaultdict'
op|'('
name|'int'
op|')'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_devs_for_part'
op|'('
name|'part'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'for'
name|'tier'
name|'in'
name|'dev'
op|'['
string|"'tiers'"
op|']'
op|':'
newline|'\n'
indent|'                    '
name|'replicas_at_tier'
op|'['
name|'tier'
op|']'
op|'+='
number|'1'
newline|'\n'
nl|'\n'
comment|'# Now, look for partitions not yet spread out enough.'
nl|'\n'
dedent|''
dedent|''
name|'undispersed_dev_replicas'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'replica'
name|'in'
name|'self'
op|'.'
name|'_replicas_for_part'
op|'('
name|'part'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'dev_id'
op|'='
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
op|'['
name|'part'
op|']'
newline|'\n'
name|'if'
name|'dev_id'
op|'=='
name|'NONE_DEV'
op|':'
newline|'\n'
indent|'                    '
name|'continue'
newline|'\n'
dedent|''
name|'dev'
op|'='
name|'self'
op|'.'
name|'devs'
op|'['
name|'dev_id'
op|']'
newline|'\n'
name|'if'
name|'all'
op|'('
name|'replicas_at_tier'
op|'['
name|'tier'
op|']'
op|'<='
nl|'\n'
name|'replica_plan'
op|'['
name|'tier'
op|']'
op|'['
string|"'max'"
op|']'
nl|'\n'
name|'for'
name|'tier'
name|'in'
name|'dev'
op|'['
string|"'tiers'"
op|']'
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'continue'
newline|'\n'
dedent|''
name|'undispersed_dev_replicas'
op|'.'
name|'append'
op|'('
op|'('
name|'dev'
op|','
name|'replica'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'not'
name|'undispersed_dev_replicas'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
nl|'\n'
dedent|''
name|'undispersed_dev_replicas'
op|'.'
name|'sort'
op|'('
nl|'\n'
name|'key'
op|'='
name|'lambda'
name|'dr'
op|':'
name|'dr'
op|'['
number|'0'
op|']'
op|'['
string|"'parts_wanted'"
op|']'
op|')'
newline|'\n'
name|'for'
name|'dev'
op|','
name|'replica'
name|'in'
name|'undispersed_dev_replicas'
op|':'
newline|'\n'
comment|'# the min part hour check is ignored iff a device has more'
nl|'\n'
comment|'# than one replica of a part assigned to it - which would have'
nl|'\n'
comment|'# only been possible on rings built with older version of code'
nl|'\n'
indent|'                '
name|'if'
op|'('
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'<'
name|'self'
op|'.'
name|'min_part_hours'
name|'and'
nl|'\n'
name|'not'
name|'replicas_at_tier'
op|'['
name|'dev'
op|'['
string|"'tiers'"
op|']'
op|'['
op|'-'
number|'1'
op|']'
op|']'
op|'>'
number|'1'
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'continue'
newline|'\n'
dedent|''
name|'dev'
op|'['
string|"'parts_wanted'"
op|']'
op|'+='
number|'1'
newline|'\n'
name|'dev'
op|'['
string|"'parts'"
op|']'
op|'-='
number|'1'
newline|'\n'
name|'assign_parts'
op|'['
name|'part'
op|']'
op|'.'
name|'append'
op|'('
name|'replica'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
nl|'\n'
string|'"Gathered %d/%d from dev %d [dispersion]"'
op|','
nl|'\n'
name|'part'
op|','
name|'replica'
op|','
name|'dev'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
op|'['
name|'part'
op|']'
op|'='
name|'NONE_DEV'
newline|'\n'
name|'for'
name|'tier'
name|'in'
name|'dev'
op|'['
string|"'tiers'"
op|']'
op|':'
newline|'\n'
indent|'                    '
name|'replicas_at_tier'
op|'['
name|'tier'
op|']'
op|'-='
number|'1'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'='
number|'0'
newline|'\n'
nl|'\n'
DECL|member|_gather_parts_for_balance_can_disperse
dedent|''
dedent|''
dedent|''
name|'def'
name|'_gather_parts_for_balance_can_disperse'
op|'('
name|'self'
op|','
name|'assign_parts'
op|','
name|'start'
op|','
nl|'\n'
name|'replica_plan'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Update the map of partition => [replicas] to be reassigned from\n        overweight drives where the replicas can be better dispersed to\n        another failure domain.\n\n        :param assign_parts: the map of partition => [replica] to update\n        :param start: offset into self.parts to begin search\n        :param replica_plan: replicanth targets for tiers\n        """'
newline|'\n'
comment|'# Last, we gather partitions from devices that are "overweight" because'
nl|'\n'
comment|'# they have more partitions than their parts_wanted.'
nl|'\n'
name|'for'
name|'offset'
name|'in'
name|'range'
op|'('
name|'self'
op|'.'
name|'parts'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'part'
op|'='
op|'('
name|'start'
op|'+'
name|'offset'
op|')'
op|'%'
name|'self'
op|'.'
name|'parts'
newline|'\n'
name|'if'
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'<'
name|'self'
op|'.'
name|'min_part_hours'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
comment|"# For each part we'll look at the devices holding those parts and"
nl|'\n'
comment|'# see if any are overweight, keeping track of replicas_at_tier as'
nl|'\n'
comment|'# we go'
nl|'\n'
dedent|''
name|'overweight_dev_replica'
op|'='
op|'['
op|']'
newline|'\n'
name|'replicas_at_tier'
op|'='
name|'defaultdict'
op|'('
name|'int'
op|')'
newline|'\n'
name|'for'
name|'replica'
name|'in'
name|'self'
op|'.'
name|'_replicas_for_part'
op|'('
name|'part'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'dev_id'
op|'='
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
op|'['
name|'part'
op|']'
newline|'\n'
name|'if'
name|'dev_id'
op|'=='
name|'NONE_DEV'
op|':'
newline|'\n'
indent|'                    '
name|'continue'
newline|'\n'
dedent|''
name|'dev'
op|'='
name|'self'
op|'.'
name|'devs'
op|'['
name|'dev_id'
op|']'
newline|'\n'
name|'for'
name|'tier'
name|'in'
name|'dev'
op|'['
string|"'tiers'"
op|']'
op|':'
newline|'\n'
indent|'                    '
name|'replicas_at_tier'
op|'['
name|'tier'
op|']'
op|'+='
number|'1'
newline|'\n'
dedent|''
name|'if'
name|'dev'
op|'['
string|"'parts_wanted'"
op|']'
op|'<'
number|'0'
op|':'
newline|'\n'
indent|'                    '
name|'overweight_dev_replica'
op|'.'
name|'append'
op|'('
op|'('
name|'dev'
op|','
name|'replica'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'not'
name|'overweight_dev_replica'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
nl|'\n'
dedent|''
name|'overweight_dev_replica'
op|'.'
name|'sort'
op|'('
nl|'\n'
name|'key'
op|'='
name|'lambda'
name|'dr'
op|':'
name|'dr'
op|'['
number|'0'
op|']'
op|'['
string|"'parts_wanted'"
op|']'
op|')'
newline|'\n'
name|'for'
name|'dev'
op|','
name|'replica'
name|'in'
name|'overweight_dev_replica'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'<'
name|'self'
op|'.'
name|'min_part_hours'
op|':'
newline|'\n'
indent|'                    '
name|'break'
newline|'\n'
dedent|''
name|'if'
name|'any'
op|'('
name|'replica_plan'
op|'['
name|'tier'
op|']'
op|'['
string|"'min'"
op|']'
op|'<='
nl|'\n'
name|'replicas_at_tier'
op|'['
name|'tier'
op|']'
op|'<'
nl|'\n'
name|'replica_plan'
op|'['
name|'tier'
op|']'
op|'['
string|"'max'"
op|']'
nl|'\n'
name|'for'
name|'tier'
name|'in'
name|'dev'
op|'['
string|"'tiers'"
op|']'
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'continue'
newline|'\n'
nl|'\n'
comment|'# this is the most overweight_device holding a replica'
nl|'\n'
comment|'# of this part that can shed it according to the plan'
nl|'\n'
dedent|''
name|'dev'
op|'['
string|"'parts_wanted'"
op|']'
op|'+='
number|'1'
newline|'\n'
name|'dev'
op|'['
string|"'parts'"
op|']'
op|'-='
number|'1'
newline|'\n'
name|'assign_parts'
op|'['
name|'part'
op|']'
op|'.'
name|'append'
op|'('
name|'replica'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
nl|'\n'
string|'"Gathered %d/%d from dev %d [weight disperse]"'
op|','
nl|'\n'
name|'part'
op|','
name|'replica'
op|','
name|'dev'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
op|'['
name|'part'
op|']'
op|'='
name|'NONE_DEV'
newline|'\n'
name|'for'
name|'tier'
name|'in'
name|'dev'
op|'['
string|"'tiers'"
op|']'
op|':'
newline|'\n'
indent|'                    '
name|'replicas_at_tier'
op|'['
name|'tier'
op|']'
op|'-='
number|'1'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'='
number|'0'
newline|'\n'
nl|'\n'
DECL|member|_gather_parts_for_balance
dedent|''
dedent|''
dedent|''
name|'def'
name|'_gather_parts_for_balance'
op|'('
name|'self'
op|','
name|'assign_parts'
op|','
name|'replica_plan'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Gather parts that look like they should move for balance reasons.\n\n        A simple gather of parts that looks dispersible normally works out,\n        we\'ll switch strategies if things don\'t be seem to moving...\n        """'
newline|'\n'
comment|'# pick a random starting point on the other side of the ring'
nl|'\n'
name|'quarter_turn'
op|'='
op|'('
name|'self'
op|'.'
name|'parts'
op|'//'
number|'4'
op|')'
newline|'\n'
name|'random_half'
op|'='
name|'random'
op|'.'
name|'randint'
op|'('
number|'0'
op|','
name|'self'
op|'.'
name|'parts'
op|'/'
number|'2'
op|')'
newline|'\n'
name|'start'
op|'='
op|'('
name|'self'
op|'.'
name|'_last_part_gather_start'
op|'+'
name|'quarter_turn'
op|'+'
nl|'\n'
name|'random_half'
op|')'
op|'%'
name|'self'
op|'.'
name|'parts'
newline|'\n'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
string|"'Gather start is %s '"
nl|'\n'
string|"'(Last start was %s)'"
op|'%'
op|'('
nl|'\n'
name|'start'
op|','
name|'self'
op|'.'
name|'_last_part_gather_start'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_last_part_gather_start'
op|'='
name|'start'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_gather_parts_for_balance_can_disperse'
op|'('
nl|'\n'
name|'assign_parts'
op|','
name|'start'
op|','
name|'replica_plan'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'assign_parts'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'_gather_parts_for_balance_forced'
op|'('
name|'assign_parts'
op|','
name|'start'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_gather_parts_for_balance_forced
dedent|''
dedent|''
name|'def'
name|'_gather_parts_for_balance_forced'
op|'('
name|'self'
op|','
name|'assign_parts'
op|','
name|'start'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Update the map of partition => [replicas] to be reassigned from\n        overweight drives without restriction, parts gathered from this method\n        may be placed back onto devices that are no better (or worse) than the\n        device from which they are gathered.\n\n        This method allows devices to flop around enough to unlock replicas\n        that would have otherwise potentially been locked because of\n        dispersion - it should be used as a last resort.\n\n        :param assign_parts: the map of partition => [replica] to update\n        :param start: offset into self.parts to begin search\n        """'
newline|'\n'
name|'for'
name|'offset'
name|'in'
name|'range'
op|'('
name|'self'
op|'.'
name|'parts'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'part'
op|'='
op|'('
name|'start'
op|'+'
name|'offset'
op|')'
op|'%'
name|'self'
op|'.'
name|'parts'
newline|'\n'
name|'if'
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'<'
name|'self'
op|'.'
name|'min_part_hours'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
dedent|''
name|'overweight_dev_replica'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'replica'
name|'in'
name|'self'
op|'.'
name|'_replicas_for_part'
op|'('
name|'part'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'dev_id'
op|'='
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
op|'['
name|'part'
op|']'
newline|'\n'
name|'if'
name|'dev_id'
op|'=='
name|'NONE_DEV'
op|':'
newline|'\n'
indent|'                    '
name|'continue'
newline|'\n'
dedent|''
name|'dev'
op|'='
name|'self'
op|'.'
name|'devs'
op|'['
name|'dev_id'
op|']'
newline|'\n'
name|'if'
name|'dev'
op|'['
string|"'parts_wanted'"
op|']'
op|'<'
number|'0'
op|':'
newline|'\n'
indent|'                    '
name|'overweight_dev_replica'
op|'.'
name|'append'
op|'('
op|'('
name|'dev'
op|','
name|'replica'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'not'
name|'overweight_dev_replica'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
nl|'\n'
dedent|''
name|'overweight_dev_replica'
op|'.'
name|'sort'
op|'('
nl|'\n'
name|'key'
op|'='
name|'lambda'
name|'dr'
op|':'
name|'dr'
op|'['
number|'0'
op|']'
op|'['
string|"'parts_wanted'"
op|']'
op|')'
newline|'\n'
name|'for'
name|'dev'
op|','
name|'replica'
name|'in'
name|'overweight_dev_replica'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'<'
name|'self'
op|'.'
name|'min_part_hours'
op|':'
newline|'\n'
indent|'                    '
name|'break'
newline|'\n'
comment|'# this is the most overweight_device holding a replica of this'
nl|'\n'
comment|"# part we don't know where it's going to end up - but we'll"
nl|'\n'
comment|'# pick it up and hope for the best.'
nl|'\n'
dedent|''
name|'dev'
op|'['
string|"'parts_wanted'"
op|']'
op|'+='
number|'1'
newline|'\n'
name|'dev'
op|'['
string|"'parts'"
op|']'
op|'-='
number|'1'
newline|'\n'
name|'assign_parts'
op|'['
name|'part'
op|']'
op|'.'
name|'append'
op|'('
name|'replica'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
nl|'\n'
string|'"Gathered %d/%d from dev %d [weight forced]"'
op|','
nl|'\n'
name|'part'
op|','
name|'replica'
op|','
name|'dev'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
op|'['
name|'part'
op|']'
op|'='
name|'NONE_DEV'
newline|'\n'
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'='
number|'0'
newline|'\n'
nl|'\n'
DECL|member|_reassign_parts
dedent|''
dedent|''
dedent|''
name|'def'
name|'_reassign_parts'
op|'('
name|'self'
op|','
name|'reassign_parts'
op|','
name|'replica_plan'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        For an existing ring data set, partitions are reassigned similarly to\n        the initial assignment.\n\n        The devices are ordered by how many partitions they still want and\n        kept in that order throughout the process.\n\n        The gathered partitions are iterated through, assigning them to\n        devices according to the "most wanted" while keeping the replicas as\n        "far apart" as possible.\n\n        Two different regions are considered the farthest-apart things,\n        followed by zones, then different ip within a zone; the\n        least-far-apart things are different devices with the same ip in the\n        same zone.\n\n        :param reassign_parts: An iterable of (part, replicas_to_replace)\n                               pairs. replicas_to_replace is an iterable of the\n                               replica (an int) to replace for that partition.\n                               replicas_to_replace may be shared for multiple\n                               partitions, so be sure you do not modify it.\n        """'
newline|'\n'
name|'parts_available_in_tier'
op|'='
name|'defaultdict'
op|'('
name|'int'
op|')'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'dev'
op|'['
string|"'sort_key'"
op|']'
op|'='
name|'self'
op|'.'
name|'_sort_key_for'
op|'('
name|'dev'
op|')'
newline|'\n'
comment|'# Note: this represents how many partitions may be assigned to a'
nl|'\n'
comment|'# given tier (region/zone/server/disk). It does not take into'
nl|'\n'
comment|'# account how many partitions a given tier wants to shed.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# If we did not do this, we could have a zone where, at some'
nl|'\n'
comment|'# point during assignment, number-of-parts-to-gain equals'
nl|'\n'
comment|'# number-of-parts-to-shed. At that point, no further placement'
nl|'\n'
comment|'# into that zone would occur since its parts_available_in_tier'
nl|'\n'
comment|'# would be 0. This would happen any time a zone had any device'
nl|'\n'
comment|'# with partitions to shed, which is any time a device is being'
nl|'\n'
comment|'# removed, which is a pretty frequent operation.'
nl|'\n'
name|'wanted'
op|'='
name|'max'
op|'('
name|'dev'
op|'['
string|"'parts_wanted'"
op|']'
op|','
number|'0'
op|')'
newline|'\n'
name|'for'
name|'tier'
name|'in'
name|'dev'
op|'['
string|"'tiers'"
op|']'
op|':'
newline|'\n'
indent|'                '
name|'parts_available_in_tier'
op|'['
name|'tier'
op|']'
op|'+='
name|'wanted'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'available_devs'
op|'='
name|'sorted'
op|'('
op|'('
name|'d'
name|'for'
name|'d'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
name|'if'
name|'d'
op|'['
string|"'weight'"
op|']'
op|')'
op|','
nl|'\n'
name|'key'
op|'='
name|'lambda'
name|'x'
op|':'
name|'x'
op|'['
string|"'sort_key'"
op|']'
op|')'
newline|'\n'
nl|'\n'
name|'tier2devs'
op|'='
name|'defaultdict'
op|'('
name|'list'
op|')'
newline|'\n'
name|'tier2sort_key'
op|'='
name|'defaultdict'
op|'('
name|'tuple'
op|')'
newline|'\n'
name|'tier2dev_sort_key'
op|'='
name|'defaultdict'
op|'('
name|'list'
op|')'
newline|'\n'
name|'max_tier_depth'
op|'='
number|'0'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'available_devs'
op|':'
newline|'\n'
indent|'            '
name|'for'
name|'tier'
name|'in'
name|'dev'
op|'['
string|"'tiers'"
op|']'
op|':'
newline|'\n'
indent|'                '
name|'tier2devs'
op|'['
name|'tier'
op|']'
op|'.'
name|'append'
op|'('
name|'dev'
op|')'
comment|'# <-- starts out sorted!'
newline|'\n'
name|'tier2dev_sort_key'
op|'['
name|'tier'
op|']'
op|'.'
name|'append'
op|'('
name|'dev'
op|'['
string|"'sort_key'"
op|']'
op|')'
newline|'\n'
name|'tier2sort_key'
op|'['
name|'tier'
op|']'
op|'='
name|'dev'
op|'['
string|"'sort_key'"
op|']'
newline|'\n'
name|'if'
name|'len'
op|'('
name|'tier'
op|')'
op|'>'
name|'max_tier_depth'
op|':'
newline|'\n'
indent|'                    '
name|'max_tier_depth'
op|'='
name|'len'
op|'('
name|'tier'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
dedent|''
name|'tier2children_sets'
op|'='
name|'build_tier_tree'
op|'('
name|'available_devs'
op|')'
newline|'\n'
name|'tier2children'
op|'='
name|'defaultdict'
op|'('
name|'list'
op|')'
newline|'\n'
name|'tier2children_sort_key'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'tiers_list'
op|'='
op|'['
op|'('
op|')'
op|']'
newline|'\n'
name|'depth'
op|'='
number|'1'
newline|'\n'
name|'while'
name|'depth'
op|'<='
name|'max_tier_depth'
op|':'
newline|'\n'
indent|'            '
name|'new_tiers_list'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'tier'
name|'in'
name|'tiers_list'
op|':'
newline|'\n'
indent|'                '
name|'child_tiers'
op|'='
name|'list'
op|'('
name|'tier2children_sets'
op|'['
name|'tier'
op|']'
op|')'
newline|'\n'
name|'child_tiers'
op|'.'
name|'sort'
op|'('
name|'key'
op|'='
name|'tier2sort_key'
op|'.'
name|'__getitem__'
op|')'
newline|'\n'
name|'tier2children'
op|'['
name|'tier'
op|']'
op|'='
name|'child_tiers'
newline|'\n'
name|'tier2children_sort_key'
op|'['
name|'tier'
op|']'
op|'='
name|'map'
op|'('
nl|'\n'
name|'tier2sort_key'
op|'.'
name|'__getitem__'
op|','
name|'child_tiers'
op|')'
newline|'\n'
name|'new_tiers_list'
op|'.'
name|'extend'
op|'('
name|'child_tiers'
op|')'
newline|'\n'
dedent|''
name|'tiers_list'
op|'='
name|'new_tiers_list'
newline|'\n'
name|'depth'
op|'+='
number|'1'
newline|'\n'
nl|'\n'
dedent|''
name|'for'
name|'part'
op|','
name|'replace_replicas'
name|'in'
name|'reassign_parts'
op|':'
newline|'\n'
comment|'# always update part_moves for min_part_hours'
nl|'\n'
indent|'            '
name|'self'
op|'.'
name|'_last_part_moves'
op|'['
name|'part'
op|']'
op|'='
number|'0'
newline|'\n'
comment|'# count up where these replicas be'
nl|'\n'
name|'replicas_at_tier'
op|'='
name|'defaultdict'
op|'('
name|'int'
op|')'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_devs_for_part'
op|'('
name|'part'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'for'
name|'tier'
name|'in'
name|'dev'
op|'['
string|"'tiers'"
op|']'
op|':'
newline|'\n'
indent|'                    '
name|'replicas_at_tier'
op|'['
name|'tier'
op|']'
op|'+='
number|'1'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'for'
name|'replica'
name|'in'
name|'replace_replicas'
op|':'
newline|'\n'
comment|'# Find a new home for this replica'
nl|'\n'
indent|'                '
name|'tier'
op|'='
op|'('
op|')'
newline|'\n'
comment|"# This used to be a cute, recursive function, but it's been"
nl|'\n'
comment|'# unrolled for performance.'
nl|'\n'
name|'depth'
op|'='
number|'1'
newline|'\n'
name|'while'
name|'depth'
op|'<='
name|'max_tier_depth'
op|':'
newline|'\n'
comment|"# Choose the roomiest tier among those that don't"
nl|'\n'
comment|'# already have their max replicas assigned according'
nl|'\n'
comment|'# to the replica_plan.'
nl|'\n'
indent|'                    '
name|'candidates'
op|'='
op|'['
name|'t'
name|'for'
name|'t'
name|'in'
name|'tier2children'
op|'['
name|'tier'
op|']'
name|'if'
nl|'\n'
name|'replicas_at_tier'
op|'['
name|'t'
op|']'
op|'<'
nl|'\n'
name|'replica_plan'
op|'['
name|'t'
op|']'
op|'['
string|"'max'"
op|']'
op|']'
newline|'\n'
nl|'\n'
name|'if'
name|'not'
name|'candidates'
op|':'
newline|'\n'
indent|'                        '
name|'raise'
name|'Exception'
op|'('
string|"'no home for %s/%s %s'"
op|'%'
op|'('
nl|'\n'
name|'part'
op|','
name|'replica'
op|','
op|'{'
name|'t'
op|':'
op|'('
nl|'\n'
name|'replicas_at_tier'
op|'['
name|'t'
op|']'
op|','
nl|'\n'
name|'replica_plan'
op|'['
name|'t'
op|']'
op|'['
string|"'max'"
op|']'
op|','
nl|'\n'
op|')'
name|'for'
name|'t'
name|'in'
name|'tier2children'
op|'['
name|'tier'
op|']'
op|'}'
op|')'
op|')'
newline|'\n'
dedent|''
name|'tier'
op|'='
name|'max'
op|'('
name|'candidates'
op|','
name|'key'
op|'='
name|'lambda'
name|'t'
op|':'
nl|'\n'
name|'parts_available_in_tier'
op|'['
name|'t'
op|']'
op|')'
newline|'\n'
nl|'\n'
name|'depth'
op|'+='
number|'1'
newline|'\n'
nl|'\n'
dedent|''
name|'dev'
op|'='
name|'tier2devs'
op|'['
name|'tier'
op|']'
op|'['
op|'-'
number|'1'
op|']'
newline|'\n'
name|'dev'
op|'['
string|"'parts_wanted'"
op|']'
op|'-='
number|'1'
newline|'\n'
name|'dev'
op|'['
string|"'parts'"
op|']'
op|'+='
number|'1'
newline|'\n'
name|'for'
name|'tier'
name|'in'
name|'dev'
op|'['
string|"'tiers'"
op|']'
op|':'
newline|'\n'
indent|'                    '
name|'parts_available_in_tier'
op|'['
name|'tier'
op|']'
op|'-='
number|'1'
newline|'\n'
name|'replicas_at_tier'
op|'['
name|'tier'
op|']'
op|'+='
number|'1'
newline|'\n'
nl|'\n'
dedent|''
name|'self'
op|'.'
name|'_replica2part2dev'
op|'['
name|'replica'
op|']'
op|'['
name|'part'
op|']'
op|'='
name|'dev'
op|'['
string|"'id'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
nl|'\n'
string|'"Placed %d/%d onto dev %d"'
op|','
name|'part'
op|','
name|'replica'
op|','
name|'dev'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
nl|'\n'
comment|'# Just to save memory and keep from accidental reuse.'
nl|'\n'
dedent|''
dedent|''
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'del'
name|'dev'
op|'['
string|"'sort_key'"
op|']'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'staticmethod'
newline|'\n'
DECL|member|_sort_key_for
name|'def'
name|'_sort_key_for'
op|'('
name|'dev'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'return'
op|'('
name|'dev'
op|'['
string|"'parts_wanted'"
op|']'
op|','
name|'random'
op|'.'
name|'randint'
op|'('
number|'0'
op|','
number|'0xFFFF'
op|')'
op|','
name|'dev'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_build_max_replicas_by_tier
dedent|''
name|'def'
name|'_build_max_replicas_by_tier'
op|'('
name|'self'
op|','
name|'bound'
op|'='
name|'math'
op|'.'
name|'ceil'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Returns a defaultdict of (tier: replica_count) for all tiers in the\n        ring excluding zero weight devices.\n\n        There will always be a () entry as the root of the structure, whose\n        replica_count will equal the ring\'s replica_count.\n\n        Then there will be (region,) entries for each region, indicating the\n        maximum number of replicas the region might have for any given\n        partition.\n\n        Next there will be (region, zone) entries for each zone, indicating\n        the maximum number of replicas in a given region and zone.  Anything\n        greater than 1 indicates a partition at slightly elevated risk, as if\n        that zone were to fail multiple replicas of that partition would be\n        unreachable.\n\n        Next there will be (region, zone, ip_port) entries for each node,\n        indicating the maximum number of replicas stored on a node in a given\n        region and zone.  Anything greater than 1 indicates a partition at\n        elevated risk, as if that ip_port were to fail multiple replicas of\n        that partition would be unreachable.\n\n        Last there will be (region, zone, ip_port, device) entries for each\n        device, indicating the maximum number of replicas the device shares\n        with other devices on the same node for any given partition.\n        Anything greater than 1 indicates a partition at serious risk, as the\n        data on that partition will not be stored distinctly at the ring\'s\n        replica_count.\n\n        Example return dict for the common SAIO setup::\n\n            {(): 3.0,\n            (1,): 3.0,\n            (1, 1): 1.0,\n            (1, 1, \'127.0.0.1:6010\'): 1.0,\n            (1, 1, \'127.0.0.1:6010\', 0): 1.0,\n            (1, 2): 1.0,\n            (1, 2, \'127.0.0.1:6020\'): 1.0,\n            (1, 2, \'127.0.0.1:6020\', 1): 1.0,\n            (1, 3): 1.0,\n            (1, 3, \'127.0.0.1:6030\'): 1.0,\n            (1, 3, \'127.0.0.1:6030\', 2): 1.0,\n            (1, 4): 1.0,\n            (1, 4, \'127.0.0.1:6040\'): 1.0,\n            (1, 4, \'127.0.0.1:6040\', 3): 1.0}\n\n        """'
newline|'\n'
comment|'# Used by walk_tree to know what entries to create for each recursive'
nl|'\n'
comment|'# call.'
nl|'\n'
name|'tier2children'
op|'='
name|'self'
op|'.'
name|'_build_tier2children'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|walk_tree
name|'def'
name|'walk_tree'
op|'('
name|'tier'
op|','
name|'replica_count'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'len'
op|'('
name|'tier'
op|')'
op|'=='
number|'4'
op|':'
newline|'\n'
comment|"# special case for device, it's not recursive"
nl|'\n'
indent|'                '
name|'replica_count'
op|'='
name|'min'
op|'('
number|'1'
op|','
name|'replica_count'
op|')'
newline|'\n'
dedent|''
name|'mr'
op|'='
op|'{'
name|'tier'
op|':'
name|'replica_count'
op|'}'
newline|'\n'
name|'if'
name|'tier'
name|'in'
name|'tier2children'
op|':'
newline|'\n'
indent|'                '
name|'subtiers'
op|'='
name|'tier2children'
op|'['
name|'tier'
op|']'
newline|'\n'
name|'for'
name|'subtier'
name|'in'
name|'subtiers'
op|':'
newline|'\n'
indent|'                    '
name|'submax'
op|'='
name|'bound'
op|'('
name|'float'
op|'('
name|'replica_count'
op|')'
op|'/'
name|'len'
op|'('
name|'subtiers'
op|')'
op|')'
newline|'\n'
name|'mr'
op|'.'
name|'update'
op|'('
name|'walk_tree'
op|'('
name|'subtier'
op|','
name|'submax'
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'mr'
newline|'\n'
dedent|''
name|'mr'
op|'='
name|'defaultdict'
op|'('
name|'float'
op|')'
newline|'\n'
name|'mr'
op|'.'
name|'update'
op|'('
name|'walk_tree'
op|'('
op|'('
op|')'
op|','
name|'self'
op|'.'
name|'replicas'
op|')'
op|')'
newline|'\n'
name|'return'
name|'mr'
newline|'\n'
nl|'\n'
DECL|member|_build_weighted_replicas_by_tier
dedent|''
name|'def'
name|'_build_weighted_replicas_by_tier'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Returns a dict mapping <tier> => replicanths for all tiers in\n        the ring based on their weights.\n        """'
newline|'\n'
name|'weight_of_one_part'
op|'='
name|'self'
op|'.'
name|'weight_of_one_part'
op|'('
op|')'
newline|'\n'
nl|'\n'
comment|"# assign each device some replicanths by weight (can't be > 1)"
nl|'\n'
name|'weighted_replicas_for_dev'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'devices_with_room'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'not'
name|'dev'
op|'['
string|"'weight'"
op|']'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
dedent|''
name|'weighted_replicas'
op|'='
op|'('
nl|'\n'
name|'dev'
op|'['
string|"'weight'"
op|']'
op|'*'
name|'weight_of_one_part'
op|'/'
name|'self'
op|'.'
name|'parts'
op|')'
newline|'\n'
name|'if'
name|'weighted_replicas'
op|'<'
number|'1'
op|':'
newline|'\n'
indent|'                '
name|'devices_with_room'
op|'.'
name|'append'
op|'('
name|'dev'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'weighted_replicas'
op|'='
number|'1'
newline|'\n'
dedent|''
name|'weighted_replicas_for_dev'
op|'['
name|'dev'
op|'['
string|"'id'"
op|']'
op|']'
op|'='
name|'weighted_replicas'
newline|'\n'
nl|'\n'
dedent|''
name|'while'
name|'True'
op|':'
newline|'\n'
indent|'            '
name|'remaining'
op|'='
name|'self'
op|'.'
name|'replicas'
op|'-'
name|'sum'
op|'('
name|'weighted_replicas_for_dev'
op|'.'
name|'values'
op|'('
op|')'
op|')'
newline|'\n'
name|'if'
name|'remaining'
op|'<'
number|'1e-10'
op|':'
newline|'\n'
indent|'                '
name|'break'
newline|'\n'
dedent|''
name|'devices_with_room'
op|'='
op|'['
name|'d'
name|'for'
name|'d'
name|'in'
name|'devices_with_room'
name|'if'
nl|'\n'
name|'weighted_replicas_for_dev'
op|'['
name|'d'
op|']'
op|'<'
number|'1'
op|']'
newline|'\n'
name|'rel_weight'
op|'='
name|'remaining'
op|'/'
name|'sum'
op|'('
nl|'\n'
name|'weighted_replicas_for_dev'
op|'['
name|'d'
op|']'
name|'for'
name|'d'
name|'in'
name|'devices_with_room'
op|')'
newline|'\n'
name|'for'
name|'d'
name|'in'
name|'devices_with_room'
op|':'
newline|'\n'
indent|'                '
name|'weighted_replicas_for_dev'
op|'['
name|'d'
op|']'
op|'='
name|'min'
op|'('
nl|'\n'
number|'1'
op|','
name|'weighted_replicas_for_dev'
op|'['
name|'d'
op|']'
op|'*'
op|'('
name|'rel_weight'
op|'+'
number|'1'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'weighted_replicas_by_tier'
op|'='
name|'defaultdict'
op|'('
name|'float'
op|')'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'not'
name|'dev'
op|'['
string|"'weight'"
op|']'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
dedent|''
name|'assigned_replicanths'
op|'='
name|'weighted_replicas_for_dev'
op|'['
name|'dev'
op|'['
string|"'id'"
op|']'
op|']'
newline|'\n'
name|'dev_tier'
op|'='
op|'('
name|'dev'
op|'['
string|"'region'"
op|']'
op|','
name|'dev'
op|'['
string|"'zone'"
op|']'
op|','
name|'dev'
op|'['
string|"'ip'"
op|']'
op|','
name|'dev'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
name|'for'
name|'i'
name|'in'
name|'range'
op|'('
name|'len'
op|'('
name|'dev_tier'
op|')'
op|'+'
number|'1'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'tier'
op|'='
name|'dev_tier'
op|'['
op|':'
name|'i'
op|']'
newline|'\n'
name|'weighted_replicas_by_tier'
op|'['
name|'tier'
op|']'
op|'+='
name|'assigned_replicanths'
newline|'\n'
nl|'\n'
comment|'# belts & suspenders/paranoia -  at every level, the sum of'
nl|'\n'
comment|'# weighted_replicas should be very close to the total number of'
nl|'\n'
comment|'# replicas for the ring'
nl|'\n'
dedent|''
dedent|''
name|'tiers'
op|'='
op|'['
string|"'cluster'"
op|','
string|"'regions'"
op|','
string|"'zones'"
op|','
string|"'servers'"
op|','
string|"'devices'"
op|']'
newline|'\n'
name|'for'
name|'i'
op|','
name|'tier_name'
name|'in'
name|'enumerate'
op|'('
name|'tiers'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'replicas_at_tier'
op|'='
name|'sum'
op|'('
name|'weighted_replicas_by_tier'
op|'['
name|'t'
op|']'
name|'for'
name|'t'
name|'in'
nl|'\n'
name|'weighted_replicas_by_tier'
name|'if'
name|'len'
op|'('
name|'t'
op|')'
op|'=='
name|'i'
op|')'
newline|'\n'
name|'if'
name|'abs'
op|'('
name|'self'
op|'.'
name|'replicas'
op|'-'
name|'replicas_at_tier'
op|')'
op|'>'
number|'1e-10'
op|':'
newline|'\n'
indent|'                '
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|"'%s != %s at tier %s'"
op|'%'
op|'('
nl|'\n'
name|'replicas_at_tier'
op|','
name|'self'
op|'.'
name|'replicas'
op|','
name|'tier_name'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'return'
name|'weighted_replicas_by_tier'
newline|'\n'
nl|'\n'
DECL|member|_build_wanted_replicas_by_tier
dedent|''
name|'def'
name|'_build_wanted_replicas_by_tier'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Returns a defaultdict of (tier: replicanths) for all tiers in the ring\n        based on unique-as-possible (full dispersion) with respect to their\n        weights and device counts.\n\n        N.B.  _build_max_replicas_by_tier calculates the upper bound on the\n        replicanths each tier may hold irrespective of the weights of the\n        tier; this method will calculate the minimum replicanth <=\n        max_replicas[tier] that will still solve dispersion.  However it is\n        not guaranteed to return a fully dispersed solution if failure domains\n        are over-weighted for their device count.\n        """'
newline|'\n'
name|'weighted_replicas'
op|'='
name|'self'
op|'.'
name|'_build_weighted_replicas_by_tier'
op|'('
op|')'
newline|'\n'
name|'dispersed_replicas'
op|'='
op|'{'
nl|'\n'
name|'t'
op|':'
op|'{'
nl|'\n'
string|"'min'"
op|':'
name|'math'
op|'.'
name|'floor'
op|'('
name|'r'
op|')'
op|','
nl|'\n'
string|"'max'"
op|':'
name|'math'
op|'.'
name|'ceil'
op|'('
name|'r'
op|')'
op|','
nl|'\n'
op|'}'
name|'for'
op|'('
name|'t'
op|','
name|'r'
op|')'
name|'in'
nl|'\n'
name|'self'
op|'.'
name|'_build_max_replicas_by_tier'
op|'('
name|'bound'
op|'='
name|'float'
op|')'
op|'.'
name|'items'
op|'('
op|')'
nl|'\n'
op|'}'
newline|'\n'
nl|'\n'
comment|'# watch out for device limited tiers'
nl|'\n'
name|'num_devices'
op|'='
name|'defaultdict'
op|'('
name|'int'
op|')'
newline|'\n'
name|'for'
name|'d'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'d'
op|'['
string|"'weight'"
op|']'
op|'<='
number|'0'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
dedent|''
name|'for'
name|'t'
name|'in'
op|'('
name|'d'
op|'.'
name|'get'
op|'('
string|"'tiers'"
op|')'
name|'or'
name|'tiers_for_dev'
op|'('
name|'d'
op|')'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'num_devices'
op|'['
name|'t'
op|']'
op|'+='
number|'1'
newline|'\n'
dedent|''
name|'num_devices'
op|'['
op|'('
op|')'
op|']'
op|'+='
number|'1'
newline|'\n'
nl|'\n'
dedent|''
name|'tier2children'
op|'='
name|'self'
op|'.'
name|'_build_tier2children'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'wanted_replicas'
op|'='
name|'defaultdict'
op|'('
name|'float'
op|')'
newline|'\n'
nl|'\n'
DECL|function|place_replicas
name|'def'
name|'place_replicas'
op|'('
name|'tier'
op|','
name|'replicanths'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'replicanths'
op|'>'
name|'num_devices'
op|'['
name|'tier'
op|']'
op|':'
newline|'\n'
indent|'                '
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|"'More than replicanths (%s) than devices (%s) '"
nl|'\n'
string|"'in tier (%s)'"
op|'%'
op|'('
name|'replicanths'
op|','
name|'num_devices'
op|'['
name|'tier'
op|']'
op|','
name|'tier'
op|')'
op|')'
newline|'\n'
dedent|''
name|'wanted_replicas'
op|'['
name|'tier'
op|']'
op|'='
name|'replicanths'
newline|'\n'
name|'sub_tiers'
op|'='
name|'sorted'
op|'('
name|'tier2children'
op|'['
name|'tier'
op|']'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'sub_tiers'
op|':'
newline|'\n'
indent|'                '
name|'return'
newline|'\n'
nl|'\n'
dedent|''
name|'to_place'
op|'='
name|'defaultdict'
op|'('
name|'float'
op|')'
newline|'\n'
name|'remaining'
op|'='
name|'replicanths'
newline|'\n'
name|'tiers_to_spread'
op|'='
name|'sub_tiers'
newline|'\n'
name|'device_limited'
op|'='
name|'False'
newline|'\n'
nl|'\n'
name|'while'
name|'True'
op|':'
newline|'\n'
indent|'                '
name|'rel_weight'
op|'='
name|'remaining'
op|'/'
name|'sum'
op|'('
name|'weighted_replicas'
op|'['
name|'t'
op|']'
nl|'\n'
name|'for'
name|'t'
name|'in'
name|'tiers_to_spread'
op|')'
newline|'\n'
name|'for'
name|'t'
name|'in'
name|'tiers_to_spread'
op|':'
newline|'\n'
indent|'                    '
name|'replicas'
op|'='
name|'to_place'
op|'['
name|'t'
op|']'
op|'+'
op|'('
nl|'\n'
name|'weighted_replicas'
op|'['
name|'t'
op|']'
op|'*'
name|'rel_weight'
op|')'
newline|'\n'
name|'if'
name|'replicas'
op|'<'
name|'dispersed_replicas'
op|'['
name|'t'
op|']'
op|'['
string|"'min'"
op|']'
op|':'
newline|'\n'
indent|'                        '
name|'replicas'
op|'='
name|'dispersed_replicas'
op|'['
name|'t'
op|']'
op|'['
string|"'min'"
op|']'
newline|'\n'
dedent|''
name|'elif'
op|'('
name|'replicas'
op|'>'
name|'dispersed_replicas'
op|'['
name|'t'
op|']'
op|'['
string|"'max'"
op|']'
name|'and'
nl|'\n'
name|'not'
name|'device_limited'
op|')'
op|':'
newline|'\n'
indent|'                        '
name|'replicas'
op|'='
name|'dispersed_replicas'
op|'['
name|'t'
op|']'
op|'['
string|"'max'"
op|']'
newline|'\n'
dedent|''
name|'if'
name|'replicas'
op|'>'
name|'num_devices'
op|'['
name|'t'
op|']'
op|':'
newline|'\n'
indent|'                        '
name|'replicas'
op|'='
name|'num_devices'
op|'['
name|'t'
op|']'
newline|'\n'
dedent|''
name|'to_place'
op|'['
name|'t'
op|']'
op|'='
name|'replicas'
newline|'\n'
nl|'\n'
dedent|''
name|'remaining'
op|'='
name|'replicanths'
op|'-'
name|'sum'
op|'('
name|'to_place'
op|'.'
name|'values'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'remaining'
op|'<'
op|'-'
number|'1e-10'
op|':'
newline|'\n'
indent|'                    '
name|'tiers_to_spread'
op|'='
op|'['
nl|'\n'
name|'t'
name|'for'
name|'t'
name|'in'
name|'sub_tiers'
nl|'\n'
name|'if'
name|'to_place'
op|'['
name|'t'
op|']'
op|'>'
name|'dispersed_replicas'
op|'['
name|'t'
op|']'
op|'['
string|"'min'"
op|']'
nl|'\n'
op|']'
newline|'\n'
dedent|''
name|'elif'
name|'remaining'
op|'>'
number|'1e-10'
op|':'
newline|'\n'
indent|'                    '
name|'tiers_to_spread'
op|'='
op|'['
nl|'\n'
name|'t'
name|'for'
name|'t'
name|'in'
name|'sub_tiers'
nl|'\n'
name|'if'
op|'('
name|'num_devices'
op|'['
name|'t'
op|']'
op|'>'
name|'to_place'
op|'['
name|'t'
op|']'
op|'<'
nl|'\n'
name|'dispersed_replicas'
op|'['
name|'t'
op|']'
op|'['
string|"'max'"
op|']'
op|')'
nl|'\n'
op|']'
newline|'\n'
name|'if'
name|'not'
name|'tiers_to_spread'
op|':'
newline|'\n'
indent|'                        '
name|'device_limited'
op|'='
name|'True'
newline|'\n'
name|'tiers_to_spread'
op|'='
op|'['
nl|'\n'
name|'t'
name|'for'
name|'t'
name|'in'
name|'sub_tiers'
nl|'\n'
name|'if'
name|'to_place'
op|'['
name|'t'
op|']'
op|'<'
name|'num_devices'
op|'['
name|'t'
op|']'
nl|'\n'
op|']'
newline|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
comment|'# remaining is "empty"'
nl|'\n'
indent|'                    '
name|'break'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'for'
name|'t'
name|'in'
name|'sub_tiers'
op|':'
newline|'\n'
indent|'                '
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
string|"'Planning %s on %s'"
op|','
nl|'\n'
name|'to_place'
op|'['
name|'t'
op|']'
op|','
name|'t'
op|')'
newline|'\n'
name|'place_replicas'
op|'('
name|'t'
op|','
name|'to_place'
op|'['
name|'t'
op|']'
op|')'
newline|'\n'
nl|'\n'
comment|'# place all replicas in the cluster tier'
nl|'\n'
dedent|''
dedent|''
name|'place_replicas'
op|'('
op|'('
op|')'
op|','
name|'self'
op|'.'
name|'replicas'
op|')'
newline|'\n'
nl|'\n'
comment|'# belts & suspenders/paranoia -  at every level, the sum of'
nl|'\n'
comment|'# wanted_replicas should be very close to the total number of'
nl|'\n'
comment|'# replicas for the ring'
nl|'\n'
name|'tiers'
op|'='
op|'['
string|"'cluster'"
op|','
string|"'regions'"
op|','
string|"'zones'"
op|','
string|"'servers'"
op|','
string|"'devices'"
op|']'
newline|'\n'
name|'for'
name|'i'
op|','
name|'tier_name'
name|'in'
name|'enumerate'
op|'('
name|'tiers'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'replicas_at_tier'
op|'='
name|'sum'
op|'('
name|'wanted_replicas'
op|'['
name|'t'
op|']'
name|'for'
name|'t'
name|'in'
nl|'\n'
name|'wanted_replicas'
name|'if'
name|'len'
op|'('
name|'t'
op|')'
op|'=='
name|'i'
op|')'
newline|'\n'
name|'if'
name|'abs'
op|'('
name|'self'
op|'.'
name|'replicas'
op|'-'
name|'replicas_at_tier'
op|')'
op|'>'
number|'1e-10'
op|':'
newline|'\n'
indent|'                '
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|"'%s != %s at tier %s'"
op|'%'
op|'('
nl|'\n'
name|'replicas_at_tier'
op|','
name|'self'
op|'.'
name|'replicas'
op|','
name|'tier_name'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'return'
name|'wanted_replicas'
newline|'\n'
nl|'\n'
DECL|member|_build_target_replicas_by_tier
dedent|''
name|'def'
name|'_build_target_replicas_by_tier'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Build a map of <tier> => <target_replicas> accounting for device\n        weights, unique-as-possible dispersion and overload.\n\n        <tier> - a tuple, describing each tier in the ring topology\n        <target_replicas> - a float, the target replicanths at the tier\n        """'
newline|'\n'
name|'weighted_replicas'
op|'='
name|'self'
op|'.'
name|'_build_weighted_replicas_by_tier'
op|'('
op|')'
newline|'\n'
name|'wanted_replicas'
op|'='
name|'self'
op|'.'
name|'_build_wanted_replicas_by_tier'
op|'('
op|')'
newline|'\n'
name|'max_overload'
op|'='
name|'self'
op|'.'
name|'get_required_overload'
op|'('
name|'weighted'
op|'='
name|'weighted_replicas'
op|','
nl|'\n'
name|'wanted'
op|'='
name|'wanted_replicas'
op|')'
newline|'\n'
name|'if'
name|'max_overload'
op|'<='
number|'0.0'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'wanted_replicas'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'overload'
op|'='
name|'min'
op|'('
name|'self'
op|'.'
name|'overload'
op|','
name|'max_overload'
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'logger'
op|'.'
name|'debug'
op|'('
string|'"Using effective overload of %f"'
op|','
name|'overload'
op|')'
newline|'\n'
name|'target_replicas'
op|'='
name|'defaultdict'
op|'('
name|'float'
op|')'
newline|'\n'
name|'for'
name|'tier'
op|','
name|'weighted'
name|'in'
name|'weighted_replicas'
op|'.'
name|'items'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'m'
op|'='
op|'('
name|'wanted_replicas'
op|'['
name|'tier'
op|']'
op|'-'
name|'weighted'
op|')'
op|'/'
name|'max_overload'
newline|'\n'
name|'target_replicas'
op|'['
name|'tier'
op|']'
op|'='
name|'m'
op|'*'
name|'overload'
op|'+'
name|'weighted'
newline|'\n'
nl|'\n'
comment|'# belts & suspenders/paranoia -  at every level, the sum of'
nl|'\n'
comment|'# target_replicas should be very close to the total number'
nl|'\n'
comment|'# of replicas for the ring'
nl|'\n'
dedent|''
name|'tiers'
op|'='
op|'['
string|"'cluster'"
op|','
string|"'regions'"
op|','
string|"'zones'"
op|','
string|"'servers'"
op|','
string|"'devices'"
op|']'
newline|'\n'
name|'for'
name|'i'
op|','
name|'tier_name'
name|'in'
name|'enumerate'
op|'('
name|'tiers'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'replicas_at_tier'
op|'='
name|'sum'
op|'('
name|'target_replicas'
op|'['
name|'t'
op|']'
name|'for'
name|'t'
name|'in'
nl|'\n'
name|'target_replicas'
name|'if'
name|'len'
op|'('
name|'t'
op|')'
op|'=='
name|'i'
op|')'
newline|'\n'
name|'if'
name|'abs'
op|'('
name|'self'
op|'.'
name|'replicas'
op|'-'
name|'replicas_at_tier'
op|')'
op|'>'
number|'1e-10'
op|':'
newline|'\n'
indent|'                '
name|'raise'
name|'exceptions'
op|'.'
name|'RingValidationError'
op|'('
nl|'\n'
string|"'%s != %s at tier %s'"
op|'%'
op|'('
nl|'\n'
name|'replicas_at_tier'
op|','
name|'self'
op|'.'
name|'replicas'
op|','
name|'tier_name'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'return'
name|'target_replicas'
newline|'\n'
nl|'\n'
DECL|member|_build_replica_plan
dedent|''
name|'def'
name|'_build_replica_plan'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Wraps return value of _build_target_replicas_by_tier to include\n        pre-calculated min and max values for each tier.\n\n        :returns: a dict, mapping <tier> => <replica_plan>, where\n                  <replica_plan> is itself a dict\n\n        <replica_plan> include at least the following keys:\n\n            min - the minimum number of replicas at the tier\n            target - the target replicanths at the tier\n            max - the maximum number of replicas at the tier\n        """'
newline|'\n'
comment|'# replica part-y planner!'
nl|'\n'
name|'target_replicas'
op|'='
name|'self'
op|'.'
name|'_build_target_replicas_by_tier'
op|'('
op|')'
newline|'\n'
name|'replica_plan'
op|'='
name|'defaultdict'
op|'('
nl|'\n'
name|'lambda'
op|':'
op|'{'
string|"'min'"
op|':'
number|'0'
op|','
string|"'target'"
op|':'
number|'0'
op|','
string|"'max'"
op|':'
number|'0'
op|'}'
op|')'
newline|'\n'
name|'replica_plan'
op|'.'
name|'update'
op|'('
op|'{'
nl|'\n'
name|'t'
op|':'
op|'{'
nl|'\n'
string|"'min'"
op|':'
name|'math'
op|'.'
name|'floor'
op|'('
name|'r'
op|'+'
number|'1e-10'
op|')'
op|','
nl|'\n'
string|"'target'"
op|':'
name|'r'
op|','
nl|'\n'
string|"'max'"
op|':'
name|'math'
op|'.'
name|'ceil'
op|'('
name|'r'
op|'-'
number|'1e-10'
op|')'
op|','
nl|'\n'
op|'}'
name|'for'
op|'('
name|'t'
op|','
name|'r'
op|')'
name|'in'
nl|'\n'
name|'target_replicas'
op|'.'
name|'items'
op|'('
op|')'
nl|'\n'
op|'}'
op|')'
newline|'\n'
name|'return'
name|'replica_plan'
newline|'\n'
nl|'\n'
DECL|member|_devs_for_part
dedent|''
name|'def'
name|'_devs_for_part'
op|'('
name|'self'
op|','
name|'part'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Returns a list of devices for a specified partition.\n\n        Deliberately includes duplicates.\n        """'
newline|'\n'
name|'if'
name|'self'
op|'.'
name|'_replica2part2dev'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'return'
op|'['
op|']'
newline|'\n'
dedent|''
name|'devs'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'part2dev'
name|'in'
name|'self'
op|'.'
name|'_replica2part2dev'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'part'
op|'>='
name|'len'
op|'('
name|'part2dev'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
dedent|''
name|'dev_id'
op|'='
name|'part2dev'
op|'['
name|'part'
op|']'
newline|'\n'
name|'if'
name|'dev_id'
op|'=='
name|'NONE_DEV'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
dedent|''
name|'devs'
op|'.'
name|'append'
op|'('
name|'self'
op|'.'
name|'devs'
op|'['
name|'dev_id'
op|']'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'devs'
newline|'\n'
nl|'\n'
DECL|member|_replicas_for_part
dedent|''
name|'def'
name|'_replicas_for_part'
op|'('
name|'self'
op|','
name|'part'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Returns a list of replicas for a specified partition.\n\n        These can be used as indices into self._replica2part2dev\n        without worrying about IndexErrors.\n        """'
newline|'\n'
name|'return'
op|'['
name|'replica'
name|'for'
name|'replica'
op|','
name|'part2dev'
nl|'\n'
name|'in'
name|'enumerate'
op|'('
name|'self'
op|'.'
name|'_replica2part2dev'
op|')'
nl|'\n'
name|'if'
name|'part'
op|'<'
name|'len'
op|'('
name|'part2dev'
op|')'
op|']'
newline|'\n'
nl|'\n'
DECL|member|_each_part_replica
dedent|''
name|'def'
name|'_each_part_replica'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Generator yielding every (partition, replica) pair in the ring.\n        """'
newline|'\n'
name|'for'
name|'replica'
op|','
name|'part2dev'
name|'in'
name|'enumerate'
op|'('
name|'self'
op|'.'
name|'_replica2part2dev'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'for'
name|'part'
name|'in'
name|'range'
op|'('
name|'len'
op|'('
name|'part2dev'
op|')'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'yield'
op|'('
name|'part'
op|','
name|'replica'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
dedent|''
op|'@'
name|'classmethod'
newline|'\n'
DECL|member|load
name|'def'
name|'load'
op|'('
name|'cls'
op|','
name|'builder_file'
op|','
name|'open'
op|'='
name|'open'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Obtain RingBuilder instance of the provided builder file\n\n        :param builder_file: path to builder file to load\n        :return: RingBuilder instance\n        """'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'fp'
op|'='
name|'open'
op|'('
name|'builder_file'
op|','
string|"'rb'"
op|')'
newline|'\n'
dedent|''
name|'except'
name|'IOError'
name|'as'
name|'e'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'e'
op|'.'
name|'errno'
op|'=='
name|'errno'
op|'.'
name|'ENOENT'
op|':'
newline|'\n'
indent|'                '
name|'raise'
name|'exceptions'
op|'.'
name|'FileNotFoundError'
op|'('
nl|'\n'
string|"'Ring Builder file does not exist: %s'"
op|'%'
name|'builder_file'
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'e'
op|'.'
name|'errno'
name|'in'
op|'['
name|'errno'
op|'.'
name|'EPERM'
op|','
name|'errno'
op|'.'
name|'EACCES'
op|']'
op|':'
newline|'\n'
indent|'                '
name|'raise'
name|'exceptions'
op|'.'
name|'PermissionError'
op|'('
nl|'\n'
string|"'Ring Builder file cannot be accessed: %s'"
op|'%'
name|'builder_file'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'raise'
newline|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'with'
name|'fp'
op|':'
newline|'\n'
indent|'                '
name|'try'
op|':'
newline|'\n'
indent|'                    '
name|'builder'
op|'='
name|'pickle'
op|'.'
name|'load'
op|'('
name|'fp'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'Exception'
op|':'
newline|'\n'
comment|'# raise error during unpickling as UnPicklingError'
nl|'\n'
indent|'                    '
name|'raise'
name|'exceptions'
op|'.'
name|'UnPicklingError'
op|'('
nl|'\n'
string|"'Ring Builder file is invalid: %s'"
op|'%'
name|'builder_file'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
dedent|''
name|'if'
name|'not'
name|'hasattr'
op|'('
name|'builder'
op|','
string|"'devs'"
op|')'
op|':'
newline|'\n'
indent|'            '
name|'builder_dict'
op|'='
name|'builder'
newline|'\n'
name|'builder'
op|'='
name|'RingBuilder'
op|'('
number|'1'
op|','
number|'1'
op|','
number|'1'
op|')'
newline|'\n'
name|'builder'
op|'.'
name|'copy_from'
op|'('
name|'builder_dict'
op|')'
newline|'\n'
dedent|''
name|'for'
name|'dev'
name|'in'
name|'builder'
op|'.'
name|'devs'
op|':'
newline|'\n'
comment|"# really old rings didn't have meta keys"
nl|'\n'
indent|'            '
name|'if'
name|'dev'
name|'and'
string|"'meta'"
name|'not'
name|'in'
name|'dev'
op|':'
newline|'\n'
indent|'                '
name|'dev'
op|'['
string|"'meta'"
op|']'
op|'='
string|"''"
newline|'\n'
comment|"# NOTE(akscram): An old ring builder file don't contain"
nl|'\n'
comment|'#                replication parameters.'
nl|'\n'
dedent|''
name|'if'
name|'dev'
op|':'
newline|'\n'
indent|'                '
name|'if'
string|"'ip'"
name|'in'
name|'dev'
op|':'
newline|'\n'
indent|'                    '
name|'dev'
op|'.'
name|'setdefault'
op|'('
string|"'replication_ip'"
op|','
name|'dev'
op|'['
string|"'ip'"
op|']'
op|')'
newline|'\n'
dedent|''
name|'if'
string|"'port'"
name|'in'
name|'dev'
op|':'
newline|'\n'
indent|'                    '
name|'dev'
op|'.'
name|'setdefault'
op|'('
string|"'replication_port'"
op|','
name|'dev'
op|'['
string|"'port'"
op|']'
op|')'
newline|'\n'
dedent|''
dedent|''
dedent|''
name|'return'
name|'builder'
newline|'\n'
nl|'\n'
DECL|member|save
dedent|''
name|'def'
name|'save'
op|'('
name|'self'
op|','
name|'builder_file'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Serialize this RingBuilder instance to disk.\n\n        :param builder_file: path to builder file to save\n        """'
newline|'\n'
name|'with'
name|'open'
op|'('
name|'builder_file'
op|','
string|"'wb'"
op|')'
name|'as'
name|'f'
op|':'
newline|'\n'
indent|'            '
name|'pickle'
op|'.'
name|'dump'
op|'('
name|'self'
op|'.'
name|'to_dict'
op|'('
op|')'
op|','
name|'f'
op|','
name|'protocol'
op|'='
number|'2'
op|')'
newline|'\n'
nl|'\n'
DECL|member|search_devs
dedent|''
dedent|''
name|'def'
name|'search_devs'
op|'('
name|'self'
op|','
name|'search_values'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Search devices by parameters.\n\n        :param search_values: a dictionary with search values to filter\n                              devices, supported parameters are id,\n                              region, zone, ip, port, replication_ip,\n                              replication_port, device, weight, meta\n\n        :returns: list of device dicts\n        """'
newline|'\n'
name|'matched_devs'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'dev'
name|'in'
name|'self'
op|'.'
name|'devs'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'not'
name|'dev'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
dedent|''
name|'matched'
op|'='
name|'True'
newline|'\n'
name|'for'
name|'key'
name|'in'
op|'('
string|"'id'"
op|','
string|"'region'"
op|','
string|"'zone'"
op|','
string|"'ip'"
op|','
string|"'port'"
op|','
string|"'replication_ip'"
op|','
nl|'\n'
string|"'replication_port'"
op|','
string|"'device'"
op|','
string|"'weight'"
op|','
string|"'meta'"
op|')'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'key'
name|'in'
name|'search_values'
op|':'
newline|'\n'
indent|'                    '
name|'value'
op|'='
name|'search_values'
op|'.'
name|'get'
op|'('
name|'key'
op|')'
newline|'\n'
name|'if'
name|'value'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'                        '
name|'if'
name|'key'
op|'=='
string|"'meta'"
op|':'
newline|'\n'
indent|'                            '
name|'if'
name|'value'
name|'not'
name|'in'
name|'dev'
op|'.'
name|'get'
op|'('
name|'key'
op|')'
op|':'
newline|'\n'
indent|'                                '
name|'matched'
op|'='
name|'False'
newline|'\n'
dedent|''
dedent|''
name|'elif'
name|'key'
op|'=='
string|"'ip'"
name|'or'
name|'key'
op|'=='
string|"'replication_ip'"
op|':'
newline|'\n'
indent|'                            '
name|'cdev'
op|'='
string|"''"
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'                                '
name|'cdev'
op|'='
name|'validate_and_normalize_address'
op|'('
nl|'\n'
name|'dev'
op|'.'
name|'get'
op|'('
name|'key'
op|','
string|"''"
op|')'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'ValueError'
op|':'
newline|'\n'
indent|'                                '
name|'pass'
newline|'\n'
dedent|''
name|'if'
name|'cdev'
op|'!='
name|'value'
op|':'
newline|'\n'
indent|'                                '
name|'matched'
op|'='
name|'False'
newline|'\n'
dedent|''
dedent|''
name|'elif'
name|'dev'
op|'.'
name|'get'
op|'('
name|'key'
op|')'
op|'!='
name|'value'
op|':'
newline|'\n'
indent|'                            '
name|'matched'
op|'='
name|'False'
newline|'\n'
dedent|''
dedent|''
dedent|''
dedent|''
name|'if'
name|'matched'
op|':'
newline|'\n'
indent|'                '
name|'matched_devs'
op|'.'
name|'append'
op|'('
name|'dev'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'matched_devs'
newline|'\n'
nl|'\n'
DECL|member|increase_partition_power
dedent|''
name|'def'
name|'increase_partition_power'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'""" Increases ring partition power by one.\n\n        Devices will be assigned to partitions like this:\n\n        OLD: 0, 3, 7, 5, 2, 1, ...\n        NEW: 0, 0, 3, 3, 7, 7, 5, 5, 2, 2, 1, 1, ...\n\n        """'
newline|'\n'
nl|'\n'
name|'new_replica2part2dev'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'replica'
name|'in'
name|'self'
op|'.'
name|'_replica2part2dev'
op|':'
newline|'\n'
indent|'            '
name|'new_replica'
op|'='
name|'array'
op|'('
string|"'H'"
op|')'
newline|'\n'
name|'for'
name|'device'
name|'in'
name|'replica'
op|':'
newline|'\n'
indent|'                '
name|'new_replica'
op|'.'
name|'append'
op|'('
name|'device'
op|')'
newline|'\n'
name|'new_replica'
op|'.'
name|'append'
op|'('
name|'device'
op|')'
comment|'# append device a second time'
newline|'\n'
dedent|''
name|'new_replica2part2dev'
op|'.'
name|'append'
op|'('
name|'new_replica'
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'_replica2part2dev'
op|'='
name|'new_replica2part2dev'
newline|'\n'
nl|'\n'
name|'for'
name|'device'
name|'in'
name|'self'
op|'.'
name|'_iter_devs'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'device'
op|'['
string|"'parts'"
op|']'
op|'*='
number|'2'
newline|'\n'
nl|'\n'
comment|'# We need to update the time when a partition has been moved the last'
nl|'\n'
comment|'# time. Since this is an array of all partitions, we need to double it'
nl|'\n'
comment|'# two'
nl|'\n'
dedent|''
name|'new_last_part_moves'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'partition'
name|'in'
name|'self'
op|'.'
name|'_last_part_moves'
op|':'
newline|'\n'
indent|'            '
name|'new_last_part_moves'
op|'.'
name|'append'
op|'('
name|'partition'
op|')'
newline|'\n'
name|'new_last_part_moves'
op|'.'
name|'append'
op|'('
name|'partition'
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'_last_part_moves'
op|'='
name|'new_last_part_moves'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'part_power'
op|'+='
number|'1'
newline|'\n'
name|'self'
op|'.'
name|'parts'
op|'*='
number|'2'
newline|'\n'
name|'self'
op|'.'
name|'version'
op|'+='
number|'1'
newline|'\n'
dedent|''
dedent|''
endmarker|''
end_unit
