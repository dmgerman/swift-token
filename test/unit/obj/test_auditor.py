...
newline|'\n'
nl|'\n'
comment|'# diskfiles for policy 0, 1, 2'
nl|'\n'
name|'self'
op|'.'
name|'disk_file'
...
newline|'\n'
nl|'\n'
DECL|member|test_audit_location_gets_quarantined
dedent|''
name|'def'
name|'test_audit_location_gets_quarantined'
op|'('
name|'self'
op|')'
op|':'
...
newline|'\n'
nl|'\n'
comment|"# instead of a datadir, we'll make a file!"
nl|'\n'
name|'mkdirs'
op|'('
name|'os'
...
newline|'\n'
nl|'\n'
comment|'# after we turn the crank ...'
nl|'\n'
name|'auditor_worker'
op|'.'
name|'object_audit'
...
newline|'\n'
nl|'\n'
comment|'# ... it should get quarantined'
nl|'\n'
name|'self'
op|'.'
name|'assertFalse'
...
newline|'\n'
nl|'\n'
DECL|member|test_rsync_tempfile_timeout_auto_option
dedent|''
name|'def'
name|'test_rsync_tempfile_timeout_auto_option'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|"# if we don't have access to the replicator config section we'll use"
nl|'\n'
comment|'# our default'
nl|'\n'
indent|'        '
...
op|')'
newline|'\n'
comment|'# if the rsync_tempfile_timeout option is set explicitly we use that'
nl|'\n'
name|'self'
op|'.'
name|'conf'
...
op|')'
newline|'\n'
comment|'# if we have a real config we can be a little smarter'
nl|'\n'
name|'config_path'
op|'='
name|'os'
...
op|')'
newline|'\n'
comment|'# the Daemon loader will hand the object-auditor config to the'
nl|'\n'
comment|'# auditor who will build the workers from it'
nl|'\n'
dedent|''
name|'conf'
op|'='
...
op|')'
newline|'\n'
comment|'# if there is no object-replicator section we still have to fall back'
nl|'\n'
comment|"# to default because we can't parse the config for that section!"
nl|'\n'
name|'self'
op|'.'
name|'assertEqual'
...
op|')'
newline|'\n'
comment|'# if the object-replicator section will parse but does not override'
nl|'\n'
comment|'# the default rsync_timeout we assume the default rsync_timeout value'
nl|'\n'
comment|'# and add 15mins'
nl|'\n'
name|'self'
op|'.'
name|'assertEqual'
...
op|')'
newline|'\n'
comment|'# if there is an object-replicator section with a rsync_timeout'
nl|'\n'
comment|"# configured we'll use that value (3600) + 900"
nl|'\n'
name|'self'
op|'.'
name|'assertEqual'
...
newline|'\n'
nl|'\n'
DECL|member|test_inprogress_rsync_tempfiles_get_cleaned_up
dedent|''
name|'def'
name|'test_inprogress_rsync_tempfiles_get_cleaned_up'
op|'('
name|'self'
op|')'
op|':'
...
name|'fail'
op|'('
string|"'Did not find .data file in %r: %r'"
op|'%'
nl|'\n'
op|'('
name|'self'
...
op|')'
newline|'\n'
comment|'# sanity check we have an extra file'
nl|'\n'
name|'rsync_files'
op|'='
name|'os'
...
newline|'\n'
nl|'\n'
comment|'# and after we turn the crank ...'
nl|'\n'
name|'auditor_worker'
op|'.'
name|'object_audit'
...
newline|'\n'
nl|'\n'
comment|"# ... we've still got the rsync file"
nl|'\n'
name|'self'
op|'.'
name|'assertEqual'
...
newline|'\n'
nl|'\n'
comment|"# and we'll keep it - depending on the rsync_tempfile_timeout"
nl|'\n'
name|'self'
op|'.'
name|'assertEqual'
...
name|'patch'
op|'('
string|"'swift.obj.auditor.time.time'"
op|','
nl|'\n'
name|'return_value'
op|'='
...
newline|'\n'
nl|'\n'
comment|'# but *tomorrow* when we run'
nl|'\n'
name|'tomorrow'
op|'='
name|'time'
...
newline|'\n'
nl|'\n'
comment|"# ... we'll totally clean that stuff up!"
nl|'\n'
dedent|''
name|'self'
op|'.'
...
newline|'\n'
nl|'\n'
comment|'# but if we have some random crazy file in there'
nl|'\n'
name|'random_crazy_file_path'
op|'='
name|'os'
op|'.'
name|'path'
...
newline|'\n'
nl|'\n'
comment|"# that's someone elses problem"
nl|'\n'
dedent|''
name|'self'
op|'.'
...
nl|'\n'
comment|'# stats from the original 3)'
nl|'\n'
name|'self'
op|'.'
name|'assertEqual'
...
newline|'\n'
nl|'\n'
DECL|function|mock_get_auditor_status
name|'def'
name|'mock_get_auditor_status'
op|'('
name|'path'
op|','
name|'logger'
...
name|'patch'
op|'('
string|"'swift.obj.diskfile.get_auditor_status'"
op|','
nl|'\n'
name|'mock_get_auditor_status'
op|')'
...
newline|'\n'
nl|'\n'
DECL|member|test_run_parallel_audit_once_failed_fork
dedent|''
name|'def'
name|'test_run_parallel_audit_once_failed_fork'
op|'('
name|'self'
op|')'
op|':'
...
op|':'
newline|'\n'
comment|'# this fork fails only on the 2nd call'
nl|'\n'
comment|"# it's enough to cause the growth of orphaned child processes"
nl|'\n'
indent|'            '
name|'if'
name|'len'
...
op|','
nl|'\n'
string|'"orphaned children left {0}, expected 0."'
nl|'\n'
op|'.'
name|'format'
op|'('
