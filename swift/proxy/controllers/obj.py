...
op|':'
newline|'\n'
comment|'# close down the stashed iter first so the ContextPool can'
nl|'\n'
comment|'# cleanup the frag queue feeding coros that may be currently'
nl|'\n'
comment|'# executing the internal_parts_iters.'
nl|'\n'
indent|'        '
name|'if'
name|'self'
...
name|'False'
op|','
name|'expected_frag_archive_size'
op|'='
name|'None'
op|')'
op|':'
...
newline|'\n'
nl|'\n'
comment|"# make sure this isn't there"
nl|'\n'
name|'headers'
op|'.'
name|'pop'
...
op|')'
newline|'\n'
comment|'# Sending the request concurrently to all nodes, and responding'
nl|'\n'
comment|"# with the first response isn't something useful for EC as all"
nl|'\n'
comment|"# nodes contain different fragments. Also EC has implemented it's"
nl|'\n'
comment|'# own specific implementation of concurrent gets to ec_ndata nodes.'
nl|'\n'
comment|"# So we don't need to  worry about plumbing and sending a"
nl|'\n'
comment|'# concurrency value to ResumingGetter.'
nl|'\n'
name|'with'
name|'ContextPool'
op|'('
...
op|':'
newline|'\n'
comment|'# catch any HTTPException response here so that we can'
nl|'\n'
comment|'# process response headers uniformly in _fix_response'
nl|'\n'
indent|'                    '
name|'resp'
op|'='
...
nl|'\n'
comment|'# sysmeta, so we copy that here (if it exists) so the client gets it.'
nl|'\n'
indent|'        '
name|'resp'
op|'.'
...
op|')'
newline|'\n'
comment|'# TODO: PyECLib <= 1.2.0 looks to return the segment info'
nl|'\n'
comment|'# different from the input for aligned data efficiency but'
nl|'\n'
comment|'# Swift never does. So calculate the fragment length Swift'
nl|'\n'
comment|'# will actually send to object sever by making two different'
nl|'\n'
comment|'# get_segment_info calls (until PyECLib fixed).'
nl|'\n'
comment|'# policy.fragment_size makes the call using segment size,'
nl|'\n'
comment|'# and the next call is to get info for the last segment'
nl|'\n'
nl|'\n'
comment|'# get number of fragments except the tail - use truncation //'
nl|'\n'
name|'num_fragments'
op|'='
name|'int'
...
newline|'\n'
nl|'\n'
comment|'# calculate the tail fragment_size by hand and add it to'
nl|'\n'
comment|'# expected_frag_size'
nl|'\n'
...
name|'_'
op|'('
string|"'Trying to get %(status_type)s status of PUT to %(path)s'"
op|')'
op|'%'
nl|'\n'
op|'{'
