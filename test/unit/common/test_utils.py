...
op|':'
newline|'\n'
comment|'# This may happen if some other not so nice test allowed stdout to'
nl|'\n'
comment|'# be caputred by daemonize w/o cleaning up after itself (i.e.'
nl|'\n'
comment|'# test_db_replicator.TestDBReplicator.test_run_once).  Normally'
nl|'\n'
comment|'# nose would clean this up for us (which works well and is'
nl|'\n'
comment|'# probably the best solution).  But when running with --nocapture,'
nl|'\n'
comment|'# this condition would cause the first print to acctually be'
nl|'\n'
comment|'# redirected to a log call and the test would fail - so we have to'
nl|'\n'
comment|'# go old school'
nl|'\n'
indent|'            '
name|'sys'
op|'.'
