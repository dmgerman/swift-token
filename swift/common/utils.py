...
number|'0'
newline|'\n'
comment|'# Indicates if FALLOCATE_RESERVE is the percentage of free space (True) or'
nl|'\n'
comment|'# the number of bytes (False).'
nl|'\n'
DECL|variable|FALLOCATE_IS_PERCENT
...
newline|'\n'
nl|'\n'
comment|'# Used by the parse_socket_string() function to validate IPv6 addresses'
nl|'\n'
...
nl|'\n'
nl|'\n'
DECL|function|config_fallocate_value
dedent|''
name|'def'
...
name|'ValueError'
op|'('
string|"'Error: %s is an invalid value for fallocate'"
nl|'\n'
string|"'_reserve.'"
op|'%'
name|'reserve_value'
...
name|'_'
op|'('
string|"'Unable to perform fsync() on directory %(dir)s:'"
nl|'\n'
string|"' %(err)s'"
op|')'
op|','
...
name|'ValueError'
op|'('
string|"'timestamp cannot be negative'"
op|')'
newline|'\n'
dedent|''
name|'if'
...
op|':'
newline|'\n'
comment|'# On Python 3, round manually using ROUND_HALF_EVEN rounding'
nl|'\n'
comment|'# method, to use the same rounding method than Python 2. Python 3'
nl|'\n'
comment|'# used a different rounding method, but Python 3.4.4 and 3.5.1 use'
nl|'\n'
comment|'# again ROUND_HALF_EVEN as Python 2.'
nl|'\n'
comment|'# See https://bugs.python.org/issue23517'
nl|'\n'
indent|'            '
name|'frac'
op|','
...
nl|'\n'
nl|'\n'
DECL|function|encode_timestamps
dedent|''
dedent|''
name|'def'
...
nl|'\n'
nl|'\n'
DECL|function|decode_timestamps
dedent|''
name|'def'
...
newline|'\n'
comment|'# TODO: some tests, e.g. in test_replicator, put float timestamps values'
nl|'\n'
comment|"# into container db's, hence this defensive check, but in real world"
nl|'\n'
comment|'# this may never happen.'
nl|'\n'
name|'if'
name|'not'
name|'isinstance'
...
op|':'
newline|'\n'
comment|'# parse time components and their signs'
nl|'\n'
comment|'# e.g. x-y+z --> parts = [x, y, z] and signs = [+1, -1, +1]'
nl|'\n'
indent|'        '
name|'neg_parts'
op|'='
...
op|')'
newline|'\n'
comment|'# if delta = 0 we want t2 = t3 = t1 in order to'
nl|'\n'
comment|'# preserve any offset in t1 - only construct a distinct'
nl|'\n'
comment|'# timestamp if there is a non-zero delta.'
nl|'\n'
name|'if'
name|'delta'
op|':'
...
newline|'\n'
nl|'\n'
comment|'# Note: this is greenthread-local storage'
nl|'\n'
...
op|':'
newline|'\n'
comment|'# We can get into a nasty situation when logs are going to syslog'
nl|'\n'
comment|'# and syslog dies.'
nl|'\n'
comment|'#'
nl|'\n'
...
comment|'#'
nl|'\n'
comment|'# (A) someone logs something'
nl|'\n'
comment|'#'
nl|'\n'
comment|"# (B) there's an exception in sending to /dev/log since syslog is"
nl|'\n'
comment|'#     not working'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# (C) logging takes that exception and writes it to stderr (see'
nl|'\n'
comment|'#     logging.Handler.handleError)'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# (D) stderr was replaced with a LoggerFileObject at process start,'
nl|'\n'
comment|'#     so the LoggerFileObject takes the provided string and tells'
nl|'\n'
comment|'#     its logger to log it (to syslog, naturally).'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# Then, steps B through D repeat until we run out of stack.'
nl|'\n'
indent|'        '
name|'if'
name|'getattr'
...
name|'_cls_thread_local'
op|','
string|"'already_called_writelines'"
op|','
name|'False'
op|')'
op|':'
...
name|'_cls_thread_local'
op|'.'
name|'already_called_writelines'
op|'='
name|'True'
newline|'\n'
name|'try'
...
newline|'\n'
nl|'\n'
comment|'# Determine if host is IPv4 or IPv6'
nl|'\n'
name|'addr_info'
op|'='
name|'None'
...
op|':'
newline|'\n'
comment|"# Don't keep the server from starting from what could be a"
nl|'\n'
comment|'# transient DNS failure.  Any hostname will get re-resolved as'
nl|'\n'
comment|'# necessary in the .sendto() calls.'
nl|'\n'
comment|"# However, we don't know if we're IPv4 or IPv6 in this case, so"
nl|'\n'
comment|'# we assume legacy IPv4.'
nl|'\n'
indent|'                '
name|'self'
op|'.'
...
newline|'\n'
nl|'\n'
comment|'# NOTE: we use the original host value, not the DNS-resolved one'
nl|'\n'
comment|"# because if host is a hostname, we don't want to cache the DNS"
nl|'\n'
comment|'# resolution for the entire lifetime of this process.  Let standard'
nl|'\n'
comment|'# name resolution caching take effect.  This should help operators use'
nl|'\n'
comment|'# DNS trickery if they want.'
nl|'\n'
dedent|''
dedent|''
name|'if'
...
op|':'
newline|'\n'
comment|'# addr_info is a list of 5-tuples with the following structure:'
nl|'\n'
comment|'#     (family, socktype, proto, canonname, sockaddr)'
nl|'\n'
comment|'# where sockaddr is the only thing of interest to us, and we only'
nl|'\n'
comment|'# use the first result.  We want to use the originally supplied'
nl|'\n'
comment|'# host (see note above) and the remainder of the variable-length'
nl|'\n'
comment|'# sockaddr: IPv4 has (address, port) while IPv6 has (address,'
nl|'\n'
comment|'# port, flow info, scope id).'
nl|'\n'
indent|'            '
name|'sockaddr'
op|'='
...
nl|'\n'
nl|'\n'
DECL|function|parse_socket_string
dedent|''
name|'def'
...
name|'default_port'
newline|'\n'
comment|"# IPv6 addresses must be between '[]'"
nl|'\n'
name|'if'
name|'socket_string'
op|'.'
...
name|'ValueError'
op|'('
string|'"IPv6 addresses must be between \'[]\'"'
op|')'
newline|'\n'
dedent|''
name|'host'
...
nl|'\n'
nl|'\n'
DECL|function|unlink_paths_older_than
dedent|''
name|'def'
name|'unlink_paths_older_than'
op|'('
...
name|'_'
op|'('
string|'"Unable to find %(section)s config section in %(conf)s"'
op|')'
op|'%'
nl|'\n'
op|'{'
...
nl|'\n'
nl|'\n'
DECL|function|extract_swift_bytes
dedent|''
name|'def'
