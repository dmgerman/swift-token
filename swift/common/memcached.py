...
op|':'
newline|'\n'
comment|"# NOTE: We do NOT place a Timeout over the MemcacheConnPool's"
nl|'\n'
comment|"# get() method. The MemcacheConnPool's create() method already"
nl|'\n'
comment|'# places a timeout around the connect() system call, which we'
nl|'\n'
comment|'# catch below. It is possible for the underlying Queue of the'
nl|'\n'
comment|'# MemcacheConnPool to be contended such that this greenlet is'
nl|'\n'
comment|'# waiting for one or more connections to be freed up. If there'
nl|'\n'
comment|'# is a particularly slow memcache server causing that problme,'
nl|'\n'
comment|'# then the IO_TIMEOUT will catch that behavior, so we do not'
nl|'\n'
comment|'# have to place a Timeout here.'
nl|'\n'
indent|'                '
name|'fp'
op|','
